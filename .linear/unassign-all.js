...
(async () => {
  ...
  const issues = ['QUI-628', 'QUI-623', 'QUI-610', 'QUI-646', 'QUI-609', 'QUI-630'];
  for (const tid of issues) {
    const r1 = await api({ query: `{issue(id:"${tid}"){id assignee{email}}}` });
    const iid = r1.data.issue.id;
    const curAssignee = r1.data.issue.assignee?.email || '(none)';
    if (curAssignee === 'adelingtoro@gmail.com') {
      ...
      console.log(`${tid}: SKIP (already ${curAssignee})`);
    }
  }
})();
