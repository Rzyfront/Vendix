...
async function main() {
    ...
    const ownerPermissions = allPermissions.filter((p) => !p.name.includes('super_admin') && // Excluir permisos de superadmin
    ...
    for (const taxCategory of taxCategories) {
        const createdTaxCategory = await prisma.tax_categories.upsert({
            ...
            create: {
                ...
                store_id: taxCategory.organization_id === techSolutionsOrg.id
                    ? techStore1.id
                    : fashionStore1.id,
            },
        });
    ...
    for (const taxCategory of taxCategories) {
        const createdTaxRate = await prisma.tax_rates.create({
            data: {
                tax_category_id: createdTaxCategories.find((t) => t.name === taxCategory.name)?.id ||
                    0,
                store_id: taxCategory.organization_id === techSolutionsOrg.id
                    ? techStore1.id
                    : fashionStore1.id,
    ...
    const categories = [
        // Categorías para Tech Solutions
        {
            name: 'Laptops',
            slug: 'laptops',
            description: 'Computadoras portátiles de diferentes marcas y especificaciones',
            organization_id: techSolutionsOrg.id,
        },
        {
            name: 'Smartphones',
    ...
    for (const category of categories) {
        const createdCategory = await prisma.categories.create({
            data: {
                ...
                store_id: category.organization_id === techSolutionsOrg.id
                    ? techStore1.id
                    : fashionStore1.id,
            },
        });
    ...
    const techAccesoriosCategory = createdCategories.find((c) => c.slug === 'accesorios' && c.organization_id === techSolutionsOrg.id);
    const techCargadoresCategory = createdCategories.find((c) => c.slug === 'cargadores-cables' &&
    ...
    const techWarehouseBog = createdLocations.find((l) => l.code === 'TECH-BOG-001');
    const techStoreBog = createdLocations.find((l) => l.code === 'TECH-BOG-STORE');
    const techWarehouseMed = createdLocations.find((l) => l.code === 'TECH-MED-001');
    const fashionWarehouse = createdLocations.find((l) => l.code === 'FASH-BOG-001');
    const fashionStore = createdLocations.find((l) => l.code === 'FASH-BOG-STORE');
    // 14. Crear productos
    ...
    const products = [
        // Productos para Tech Solutions
        {
            ...
            organization_id: techSolutionsOrg.id,
            store_id: techStore1.id,
            category_id: createdCategories.find((c) => c.slug === 'laptops' && c.organization_id === techSolutionsOrg.id)?.id,
            brand_id: createdBrands.find((b) => b.slug === 'apple' && b.organization_id === techSolutionsOrg.id)?.id,
            status: 'active',
        },
        {
            name: 'iPhone 15 Pro',
            ...
            organization_id: techSolutionsOrg.id,
            store_id: techStore1.id,
            category_id: createdCategories.find((c) => c.slug === 'smartphones' && c.organization_id === techSolutionsOrg.id)?.id,
            brand_id: createdBrands.find((b) => b.slug === 'apple' && b.organization_id === techSolutionsOrg.id)?.id,
            status: 'active',
        },
        {
            ...
            store_id: techStore1.id,
            category_id: createdCategories.find((c) => c.slug === 'smartphones' && c.organization_id === techSolutionsOrg.id)?.id,
            brand_id: createdBrands.find((b) => b.slug === 'samsung' && b.organization_id === techSolutionsOrg.id)?.id,
            status: 'active',
        },
        {
            ...
            organization_id: techSolutionsOrg.id,
            store_id: techStore1.id,
            category_id: createdCategories.find((c) => c.slug === 'cargadores-cables' &&
                c.organization_id === techSolutionsOrg.id)?.id,
            brand_id: createdBrands.find((b) => b.slug === 'samsung' && b.organization_id === techSolutionsOrg.id)?.id,
            status: 'active',
        },
        // Productos para Fashion Retail
        {
            ...
            organization_id: fashionRetailOrg.id,
            store_id: fashionStore1.id,
            category_id: createdCategories.find((c) => c.slug === 'calzado' && c.organization_id === fashionRetailOrg.id)?.id,
            brand_id: createdBrands.find((b) => b.slug === 'nike' && b.organization_id === fashionRetailOrg.id)?.id,
            status: 'active',
        },
        {
            ...
            organization_id: fashionRetailOrg.id,
            store_id: fashionStore1.id,
            category_id: createdCategories.find((c) => c.slug === 'ropa-masculina' &&
                c.organization_id === fashionRetailOrg.id)?.id,
            brand_id: createdBrands.find((b) => b.slug === 'adidas' && b.organization_id === fashionRetailOrg.id)?.id,
            status: 'active',
        },
    ];
    ...
    for (const product of products) {
        ...
        if (createdProduct) {
            const taxCategory = createdTaxCategories.find((t) => product.organization_id === techSolutionsOrg.id
                ? t.name === 'IVA General'
    ...
    const macbookBatch = createdBatches.find((b) => b.batch_number === 'BATCH-MBP-2024-001');
    if (macbookBatch) {
        for (let i = 1; i <= 20; i++) {
            serialNumbers.push({
    ...
    const iphoneBatch = createdBatches.find((b) => b.batch_number === 'BATCH-IP15-2024-001');
    if (iphoneBatch) {
        for (let i = 1; i <= 25; i++) {
            serialNumbers.push({
    ...
    const inventoryTransactions = [
        {
            ...
            reference_id: createdBatches.find((b) => b.batch_number === 'BATCH-MBP-2024-001')?.id,
            notes: 'Stock inicial MacBook Pro',
            unit_cost: 3500000,
            batch_id: createdBatches.find((b) => b.batch_number === 'BATCH-MBP-2024-001')?.id,
        },
        {
            product_id: createdProducts.find((p) => p.sku === 'IP15P-256-BLK')?.id,
            product_variant_id: null,
            ...
            quantity: 25,
            reference_type: 'BATCH',
            reference_id: createdBatches.find((b) => b.batch_number === 'BATCH-IP15-2024-001')?.id,
            notes: 'Stock inicial iPhone 15 Pro',
            unit_cost: 4200000,
            batch_id: createdBatches.find((b) => b.batch_number === 'BATCH-IP15-2024-001')?.id,
        },
        {
            product_id: createdProducts.find((p) => p.sku === 'CHG-USB65-GAN')?.id,
            product_variant_id: null,
            ...
            quantity: 80,
            reference_type: 'BATCH',
            reference_id: createdBatches.find((b) => b.batch_number === 'BATCH-CHG-2024-001')?.id,
            notes: 'Stock inicial cargadores USB-C',
            unit_cost: 80000,
            batch_id: createdBatches.find((b) => b.batch_number === 'BATCH-CHG-2024-001')?.id,
        },
        {
            product_id: createdProducts.find((p) => p.sku === 'NAM90-42-BLK')?.id,
            product_variant_id: null,
            ...
            quantity: 50,
            reference_type: 'BATCH',
            reference_id: createdBatches.find((b) => b.batch_number === 'BATCH-NIKE-2024-001')?.id,
            notes: 'Stock inicial Nike Air Max 90',
            unit_cost: 220000,
            batch_id: createdBatches.find((b) => b.batch_number === 'BATCH-NIKE-2024-001')?.id,
        },
    ];
    ...
    console.log(`📋 Transacciones de inventario: ${inventoryTransactions.length}`);
    console.log('');
