 ...
 * ---
 *
 * ADR-16 (CP-pos-exclusive-tax-double-charge): este archivo es el KERNEL
 * único. Vive en `@common/money-kernel` (sin NestJS/Prisma/Angular) para que
 * backend y frontend compartan la MISMA aritmética; `Prisma.Decimal` es
 * `decimal.js` por dentro, así que la única diferencia real con el código
 * histórico es que acá el constructor `Decimal` es el propio del kernel
 * (`./decimal`, congelado con la config de `Prisma.Decimal` — ver ese
 * archivo), no el de `@prisma/client`. Las funciones aceptan indistintamente
 * instancias de `Prisma.Decimal` o del `Decimal` de este kernel: ambas son
 * `decimal.js` por debajo y se reconocen entre sí vía `Decimal.isDecimal()`
 * (chequeo por *tag*, no por `instanceof`) — ver la nota en {@link toDecimal}.
 * `apps/backend/.../dian-money.util.ts` es ahora una fachada que re-exporta
 * este módulo tal cual, para que sus ~30 importadores no cambien.
 *
 * @see docs/facturacion-electronica-dian-software-propio.md §20.0-bis
 */

/** Truncate toward zero — Anexo 1.9 §11.2 forbids rounding. */
const TRUNCATE = Decimal.ROUND_DOWN;
...
export type DianNumericInput = string | number | Decimal | null | undefined;
...
export function dianArithmetic(
  ...
  const total = operands.reduce<Decimal>((acc, operand) => {
...
export function dianPriceAmount(line: DianLineAmounts): string {
  ...
  return toDecimal(two_decimals).equals(per_unit)
    ? two_decimals
    : per_unit.toFixed(DIAN_PRICE_SCALE, Decimal.ROUND_UP);
}
...
export function dianLineExtensionTotal(lines: DianLineAmounts[]): string {
  const total = lines.reduce<Decimal>(
...
export function clearInclusiveLine(
  ...
  return {
    unit_price: exact_unit_price.toFixed(DIAN_PRICE_SCALE, Decimal.ROUND_UP),
...
// + report path.
//
// Decimal-space (F-041): the loop runs on integer cents (`Decimal`
...
export function absorbRateToFraction(
...
): { fraction: Decimal; basis: AbsorbRateBasis; invalid: string | null } {
  ...
  if (!isStrictFiniteInput(rate)) {
    return { fraction: new Decimal(0), basis, invalid: invalid_basis ?? 'rate:non_finite' };
  }
...
/**
 * Strict finiteness WITHOUT legacy collapsing (F-036, F-062). Frontera: acepta
 * `unknown`.
 *
 * `Decimal.isDecimal(value)` en vez de `instanceof Decimal`: acepta por igual
 * una instancia de `Prisma.Decimal` (lo que persiste la base de datos) y una
 * instancia del `Decimal` de este kernel — ambas son `decimal.js` por debajo
 * y decimal.js las reconoce entre sí por un *tag* interno
 * (`obj.toStringTag === '[object Decimal]'`), no por identidad de
 * constructor. Es justo la situación de "dos constructores, un solo tipo de
 * dato" que existe mientras backend y frontend comparten este kernel pero
 * `invoice_taxes`, `invoices`, etc. siguen siendo columnas `Prisma.Decimal`.
 */
export function isStrictFiniteInput(value: DianNumericInput | unknown): boolean {
  if (value === null || value === undefined || value === '') return false;
  if (Decimal.isDecimal(value)) return (value as Decimal).isFinite();
...
export function absorbPriceUnitDivisor(
  ...
  if (value === null || value === undefined) return new Decimal(1);
  ...
  return parsed.greaterThan(1) ? parsed : new Decimal(1);
}

function basisDivisor(basis: AbsorbRateBasis): Decimal {
  switch (basis) {
    case 'per_mil':
...
function truncCents(value: Decimal): Decimal {
...
function searchAbsorbCents(
  bruto_cents: Decimal,
  fractions: Decimal[],
): { base_cents: Decimal; steps: number; capped: boolean; closed: boolean } {
  const rate_sum = fractions.reduce<Decimal>(
    (acc, f) => acc.plus(f),
    new Decimal(0),
  );
  const quotaOf = (base_cents: Decimal, f: Decimal): Decimal =>
    base_cents.times(f).toDecimalPlaces(0, TRUNCATE);
  const fOf = (base_cents: Decimal): Decimal =>
    fractions.reduce<Decimal>(
      (acc, fraction) => acc.plus(quotaOf(base_cents, fraction)),
      base_cents,
    );

  let best = bruto_cents.dividedBy(new Decimal(1).plus(rate_sum)).toDecimalPlaces(0, TRUNCATE);
  if (best.isNegative()) best = new Decimal(0);
...
export function absorbInclusiveLine(input: AbsorbKernelLineInput): AbsorbKernelResult {
  ...
  const gross =
    gross_valid && !negative_net
      ? gross_decimal.toDecimalPlaces(DIAN_SCALE, Decimal.ROUND_HALF_UP)
      : new Decimal(0);
  ...
  const quotas: AbsorbKernelQuota[] = (input.rates ?? []).map((r, index) => {
    ...
    return { fraction, rate_basis: basis, tax_type, is_inclusive, quota: new Decimal(0) };
  });
  ...
  const finish = (
    ...
    const inclusive_sum = settled.reduce<Decimal>(
      (acc, q) => (q.is_inclusive ? acc.plus(q.quota) : acc),
      new Decimal(0),
    );
    const all_sum = settled.reduce<Decimal>((acc, q) => acc.plus(q.quota), new Decimal(0));
  ...
  const divisor = new Decimal(1).plus(
    inclusive_fractions.reduce<Decimal>((acc, f) => acc.plus(f), new Decimal(0)),
  );
  ...
  const sane_divisor = divisor.greaterThan(0) ? divisor : new Decimal(1);
 ...
 *
 * Ver la nota de {@link isStrictFiniteInput} sobre `Decimal.isDecimal()`: una
 * instancia de `Prisma.Decimal` pasa este chequeo igual que una de este
 * kernel.
 */
export function toDecimal(value: DianNumericInput | unknown): Decimal {
  ...
  if (Decimal.isDecimal(value)) {
    const decimal_value = value as Decimal;
    return decimal_value.isFinite() ? decimal_value : new Decimal(0);
  }

  try {
    const parsed = new Decimal(value as string | number);
...
function lineExtensionDecimal(line: DianLineAmounts): Decimal {
  return lineGrossDecimal(line).minus(toDecimal(line.discount_amount));
}

function lineGrossDecimal(line: DianLineAmounts): Decimal {
 ...
 */
function priceUnitDivisor(value: DianNumericInput): Decimal {
  const n = toDecimal(value);
  return n.greaterThan(1) ? n : new Decimal(1);
}
...
function applyScale(value: Decimal, scale: number = DIAN_SCALE): string {
...
const INCLUSIVE_CENT = new Decimal('0.01');
const INCLUSIVE_ZERO = new Decimal(0);
const INCLUSIVE_ONE = new Decimal(1);
const INCLUSIVE_HUNDRED = new Decimal(100);
const INCLUSIVE_THOUSAND = new Decimal(1000);
 ...
 * ```
 */
export function toFraction(rate: unknown, basis?: unknown): Decimal {
...
export function resolveInclusiveClearing(
  ...
  if (rawGross === null) {
    ...
    G = new Decimal(rawGross.toFixed(DIAN_SCALE, Decimal.ROUND_HALF_UP));
  }
  ...
  const parsed = (ratesInput ?? []).map((entry, index) => {
    ...
    let fixedBase: Decimal | null = null;
    if (is_inclusive) {
  ...
  // inclusivo).
  const fOf = (candidate: Decimal): Decimal => {
  ...
  if (
    ...
    throw new Error(
      `[money-kernel] inclusive solver exceeded bound (${iterations} steps, still converging)`,
    );
  }
  if (fOf(base).greaterThan(G) && fixedTotal.lessThanOrEqualTo(G)) {
    throw new Error('[money-kernel] inclusive solver left an infeasible base');
  }
 ...
 *
 * `Decimal.isDecimal()` en vez de `instanceof` — ver la nota en
 * {@link isStrictFiniteInput}.
 */
function parseInclusiveNumeric(value: unknown): Decimal | null {
  if (Decimal.isDecimal(value)) {
    const decimal_value = value as Decimal;
    return decimal_value.isFinite() ? decimal_value : null;
  }
...
function truncInclusive(d: Decimal): Decimal {
  return new Decimal(d.toFixed(DIAN_SCALE, TRUNCATE));
}
