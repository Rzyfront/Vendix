...
import { INCLUSIVE_SOLVER_MAX_STEPS, resolveInclusiveClearing } from './dian-money';
import type { InclusiveRateBasis } from './dian-money';
 ...
 * `resolveInclusiveClearing` (`./dian-money`, hoja sin imports de dominio —
 * por eso no hay ciclo taxes ↔ invoicing) y solo adapta `Decimal → number`.
 * Misma cota, mismas precondiciones, mismo cierre en centavos y misma
 * semántica de carve-outs donde aplica:
 ...
 *
 * ADR-16: este archivo vive en `@common/money-kernel`. La fachada
 * `apps/backend/.../taxes/utils/tax-inclusive-math.util.ts` lo re-exporta tal
 * cual para que sus importadores no cambien.
 */
...
function truncate(d: Decimal): Decimal {
  return new Decimal(d.toFixed(2, Decimal.ROUND_DOWN));
}
