...
export class AuditInterceptor implements NestInterceptor {
      ...
      tap(async (data) => {
        try {
          ...
          if (method === 'GET') {
            ...
            this.isUpdateOperation(url, method)
          ) {
            await this.logUpdateOperation(
              userId,
              organizationId,
              storeId,
              url,
  ...
  private getOrganizationId(request: any): number | undefined {
    ...
    const bodyOrgId =
      request.body?.organization_id || request.body?.organizationId;
    const queryOrgId =
      request.query?.organization_id || request.query?.organizationId;
    const paramOrgId =
      request.params?.organization_id || request.params?.organizationId;
  ...
  private getStoreId(request: any): number | undefined {
    ...
    const paramStoreId =
      request.params?.store_id || request.params?.storeId || request.params?.id; // En rutas de stores, :id suele ser el storeId
    ...
    if (paramStoreId && request.url.includes('/stores/'))
      return parseInt(paramStoreId);
  ...
  private isDeleteOperation(url: string, method: string): boolean {
    return (
      method === 'DELETE' || url.includes('/delete') || url.includes('/remove')
    );
  }
  ...
  private async logUpdateOperation(
    ...
    const resourceId =
      this.extractIdFromUrl(url) || actualNewData?.id || oldData?.id;
  ...
  private extractIdFromUrl(url: string): number | null {
    ...
    for (let i = segments.length - 1; i >= 0; i--) {
      ...
      if (!isNaN(id) && segments[i].length < 10) {
        // Simple check to avoid some large numbers/timestamps if any
        return id;
      }
    }
    return null;
