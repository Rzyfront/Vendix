...
/**
 * Phase 9 — Daily cron (06:00 UTC) that closes the gym membership retention
 * loop with two complementary passes over `gym_memberships`:
 *
 *  1. **Pre-expiry notice** (`gym_membership_expiring`): warn the store about
 ...
 *
 *  2. **Post-expiry transition** (`gym_membership_expired`): memberships still
 *     flagged `active` past their `period_end` are transitioned to
 *     `status='expired'` and a `gym_membership_expired` notification is
 *     broadcast.
 *
 * Idempotency:
 *  - Pass 1 dedups per `(membership_id, period_end)` by checking for an existing
 *    `gym_membership_expiring` notification with the same `data.membership_id`
 ...
 */
@Injectable()
export class GymMembershipExpiryNotifierJob {
  private readonly logger = new Logger(GymMembershipExpiryNotifierJob.name);
  ...
  async handleExpiryNotifications(): Promise<void> {
    if (this.isRunning) {
      this.logger.warn(
        'GymMembershipExpiryNotifierJob already running, skipping',
      );
      return;
    }
    this.isRunning = true;
    try {
      ...
      this.logger.log(
        `GYM_MEMBERSHIP_EXPIRY_BATCH expiring_notified=${expiring} expired_transitioned=${expired}`,
      );
    } catch (err: any) {
      this.logger.error(
        `Gym membership expiry notifier batch failed: ${err?.message ?? err}`,
        err?.stack,
      );
    } finally {
  ...
  async runExpiringPass(): Promise<number> {
    ...
    const windowEnd = new Date(
      now.getTime() +
        GymMembershipExpiryNotifierJob.EXPIRING_WINDOW_DAYS *
          24 *
          60 *
          60 *
          1000,
    ...
    const candidates = await this.prisma.withoutScope().gym_memberships.findMany(
      {
        where: {
    ...
    for (const m of candidates) {
      try {
        ...
        this.logger.error(
          `Gym expiring notice failed for membership ${m.id}: ${perRowErr?.message ?? perRowErr}`,
          perRowErr?.stack,
        );
      }
    }
   ...
   * broadcast `gym_membership_expired`. Visible for tests. Returns the count of
    ...
    for (const m of candidates) {
      try {
        ...
        const updated = await this.prisma
          .withoutScope()
          .gym_memberships.updateMany({
            where: { id: m.id, status: gym_membership_status_enum.active },
            data: { status: gym_membership_status_enum.expired },
          });
        ...
        this.logger.log(
          `GYM_MEMBERSHIP_EXPIRED membership_id=${m.id} store_id=${m.store_id}`,
        );
      } catch (perRowErr: any) {
        this.logger.error(
          `Gym expired transition failed for membership ${m.id}: ${perRowErr?.message ?? perRowErr}`,
          perRowErr?.stack,
        );
      }
    }
