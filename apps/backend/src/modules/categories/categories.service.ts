...
export class CategoriesService {
  ...
  async findOne(id: number, options: { includeInactive?: boolean } = {}) {
    ...
    const category = await this.prisma.categories.findFirst({ where, include: { stores: true } });
  ...
  async update(id: number, updateCategoryDto: UpdateCategoryDto, user: any) {
    ...
    if (category.store_id) await this.validateStoreAccess(category.store_id, user);
  ...
  async remove(id: number, user: any) {
    ...
    const productCount = await this.prisma.product_categories.count({ where: { category_id: id } });
  ...
  private async validateUniqueSlug(slug: string, storeId: number, excludeId?: number) {
