...
export class AuthService {
  ...
  async getOnboardingStatus(userId: number): Promise<{
    ...
    const hasOrganization = user.organization_users.length > 0;
  ...
  async createOrganizationDuringOnboarding(
    ...
    await this.prismaService.organization_users.create({
      data: {
  ...
  async setupOrganization(
    ...
    const userOrg = await this.prismaService.organization_users.findFirst({
      where: {
        user_id: userId,
