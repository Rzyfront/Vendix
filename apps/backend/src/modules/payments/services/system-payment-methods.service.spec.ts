...
describe('SystemPaymentMethodsService', () => {
    ...
    describe('update', () => {
        it('should update a system payment method', async () => {
            ...
            const updatedMethod = { ...mockSystemPaymentMethod, ...updateDto, updated_at: new Date() };

            mockPrismaService.system_payment_methods.findUnique.mockResolvedValue(mockSystemPaymentMethod);
            mockPrismaService.system_payment_methods.update.mockResolvedValue(updatedMethod);
    ...
    describe('remove', () => {
        it('should delete a system payment method', async () => {
            mockPrismaService.system_payment_methods.findUnique.mockResolvedValue(mockSystemPaymentMethod);
            mockPrismaService.system_payment_methods.delete.mockResolvedValue(mockSystemPaymentMethod);

            const result = await service.remove(1, mockUser);

            expect(result).toEqual({ success: true, message: 'System payment method deleted' });
        });
    });
});
