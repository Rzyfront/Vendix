...
export class EcommerceReservationsController {
  ...
  async createBooking(
    @Req() req: any,
    @Body() dto: CreateEcommerceBookingDto,
  ) {
    const customerId = req.user?.id;
    if (!customerId) {
      throw new ForbiddenException('Debe iniciar sesion para crear una reserva');
    }
  ...
  async cancelBooking(
    ...
    const customerId = req.user?.id;
    if (!customerId) {
      throw new ForbiddenException('Debe iniciar sesion para cancelar una reserva');
    }

    const booking = await this.reservationsService.findOne(id);
    if (booking.customer_id !== customerId) {
      throw new ForbiddenException('No tiene permiso para cancelar esta reserva');
    }
  ...
  async rescheduleBooking(
    ...
    const customerId = req.user?.id;
    if (!customerId) {
      throw new ForbiddenException('Debe iniciar sesion para reprogramar una reserva');
    }

    const booking = await this.reservationsService.findOne(id);
    if (booking.customer_id !== customerId) {
      throw new ForbiddenException('No tiene permiso para reprogramar esta reserva');
    }
