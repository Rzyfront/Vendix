...
import {
  ...
  DIAN_TAX_NAMES,
  isDianTaxSchemeCode,
} from '../../../store/invoicing/providers/dian-direct/constants/dian-tax-codes';
import {
  DIAN_DOCUMENT_TYPES,
  DIAN_INVOICE_OPERATION_TYPES,
} from '../../../store/invoicing/providers/dian-direct/constants/dian-document-types';
import { DIAN_UNIT_CODES } from '../../../store/invoicing/providers/dian-direct/constants/dian-unit-codes';
import {
  DianCustomerData,
  DianInvoiceControl,
  DianIssuerData,
  DianSoftwareSecurity,
} from '../../../store/invoicing/providers/dian-direct/interfaces/dian-config.interface';
import {
  buildAiuNote,
...
import { UblInvoiceBuilder } from '../../../store/invoicing/providers/dian-direct/xml/ubl-invoice.builder';
import {
  DianTotalsValidator,
  DianTotalsViolation,
} from '../../../store/invoicing/providers/dian-direct/xml/dian-totals.validator';
import {
  UblStructureValidator,
  UblStructureViolation,
} from '../../../store/invoicing/providers/dian-direct/xml/ubl-structure.validator';
import { ProviderInvoiceTax } from '../../../store/invoicing/providers/invoice-provider.interface';
import {
  AiuComponent,
  CalculatedAiu,
  CalculatedLine,
...
} from '../../../store/invoicing/services/invoice-calculator.service';
import { dianAmount, toDecimal } from '../../../store/invoicing/utils/dian-money.util';
...
} from '../../../store/invoicing/profiles/invoice-profile-config.contract';
import {
  PREVIEW_CUFE,
  PREVIEW_INVOICE_NUMBER,
} from '../../../store/invoicing/profiles/preview-numbering.guard';
import {
  PreviewProfileDto,
  PreviewProfileLineDto,
} from '../../../store/invoicing/profiles/dto/preview-profile.dto';
 ...
 */
/**
 * PlatformProfilePreviewService — preview org-scoped del XML proyectado (B.4).
 *
 * Espejo de `ProfilePreviewService` del riel tienda (1.850 líneas probadas) pero
 * con ámbito `organization_id` en vez de `store_id`. Reutiliza el mismo cálculo
 * (`InvoiceCalculatorService.calculate`) y el mismo constructor UBL
 * (`UblInvoiceBuilder`), y **no toca `invoice_resolutions` en absoluto** — ni para
 * leer. Eso hace que el invariante sea más fuerte que «no escribe»: la tabla no
 * participa y `current_number` no se mueve por previsualizar.
 *
 * - Carga el perfil vía `PlatformProfilesService.findOne(id)` para respetar el
 *   filtro `organization_id + store_id IS NULL` (ADR-1/ADR-4).
 * - Usa `InvoiceCalculatorService.calculate` para totales (misma aritmética que
 *   la emisión real).
 * - Respeta `PreviewNumberingGuard` logic: emite `PREVIEW_INVOICE_NUMBER` y
 *   `PREVIEW_CUFE`, nunca un consecutivo real. Si algún camino pidiera
 *   `InvoiceNumberGenerator.generateNextNumber`, el token del módulo de perfiles
 *   está sustituido por `PreviewNumberingGuard` y la llamada falla con
 *   `INVOICING_PREVIEW_001` en vez de mover `current_number`.
 * - Devuelve el mismo shape que el preview de tienda con `not_performed: true`.
 */
@Injectable()
export class PlatformProfilePreviewService {
  private readonly logger = new Logger(PlatformProfilePreviewService.name);

  constructor(
    private readonly profiles: PlatformProfilesService,
   ...
   * perfil `profile_id` plataforma.
   *
   * El perfil se lee por `PlatformProfilesService.findOne`, que ya aplica el
   * ámbito org-scoped (`organization_id + store_id IS NULL`) y responde 404
   * indistinguible para «no existe» y «es de otra organización». No se replica
   * esa lectura acá: hacerlo abriría un segundo camino de acceso al perfil con
   * su propia posibilidad de olvidar el filtro. El preview **no incrementa
   * `invoice_resolutions.current_number`** — emite `PREVIEW_INVOICE_NUMBER` fijo
   * y no toca la tabla de resoluciones (PreviewNumberingGuard logic).
   */
  async preview(
    profile_id: number,
