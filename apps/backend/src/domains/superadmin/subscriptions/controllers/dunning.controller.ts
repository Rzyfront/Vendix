...
export class DunningController {
  ...
  @Get()
  @ApiOperation({ summary: 'List subscriptions in dunning (grace/suspended/blocked)' })
  async findAll(@Query() query: DunningQueryDto) {
    const result = await this.dunningService.findAll(query);
    return this.responseService.paginated(result.data, result.meta.total, result.meta.page, result.meta.limit, 'Dunning subscriptions retrieved');
  }
