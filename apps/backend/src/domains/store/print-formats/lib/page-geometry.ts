 ...
 * geometry as `PRINT_PAGE_GEOMETRY` for the backend.
 *
 ...
 * `@vendix/print-formats` import — every app reads its own LOCAL copy.
 *
 * Why the alias `PRINT_PAGE_GEOMETRY`: the rest of the backend
 * (`document-print.service.ts`, `paper-defaults.ts`, the settings interface
 * consumers) already imports the constant under that name. We re-export the
 * same shape (with `page_size` instead of the canonical `css_page_size`) so
