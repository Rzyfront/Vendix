...
export class ProfileAccountingValidator {
  ...
  async assertAccountsUsable(
    ...
    const refs = collectAccountRefs(config);
    ...
    const issues: ProfileAccountIssue[] = [];
    if (accounting_entity_id === null) {
      ...
      // elegir.
      for (const ref of refs) issues.push(notInChart(ref, puc_label));
    } else {
      ...
      for (const ref of refs) {
        ...
        if (accepts_entries === undefined) {
          issues.push(notInChart(ref, puc_label));
        } else if (!accepts_entries) {
          // La mitad del invariante que DB-07 encontró violada en silencio: una
          // cuenta de agrupación EXISTE y no sirve — guardarla produce un
          // asiento imposible en la emisión, no en el guardado.
          issues.push({
            field: ref.field,
