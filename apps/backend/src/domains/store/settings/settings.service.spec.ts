...
describe('SettingsService — guard de transición de caja (QUI-560)', () => {
  ...
  /**
   * QUI-563 — write integrity guard. Si el cliente envía un
   * `expected_store_id` que NO coincide con el store_id del JWT activo,
   * `updateSettings` debe rechazar con 409 `STORE_SETTINGS_STORE_MISMATCH_001`
   * y NO debe llegar a `prisma.store_settings.upsert`. El frontend usa el
   * mismo header para evitar el cross-write silencioso (form de tienda A con
   * token de tienda B).
   *
   * Cobertura:
   *  - mismatch → 409 + sin upsert
   *  - match → persiste normalmente
   *  - sin header → no se valida (backwards compat)
   */
  describe('updateSettings — guard de integridad QUI-563', () => {
    it('rechaza cuando expected_store_id difiere del tenant activo', async () => {
      sessionsService.countOpenSessions.mockResolvedValue(NO_OPEN_SESSIONS);

      await expect(
        service.updateSettings({
          expected_store_id: STORE_ID + 1,
          pos: { cash_register: { enabled: false } },
        } as any),
      ).rejects.toMatchObject({ errorCode: 'STORE_SETTINGS_STORE_MISMATCH_001' });

      expect(prisma.store_settings.upsert).not.toHaveBeenCalled();
    });

    it('acepta cuando expected_store_id coincide con el tenant activo', async () => {
      sessionsService.countOpenSessions.mockResolvedValue(NO_OPEN_SESSIONS);
    ...
    it('no aplica el guard si el cliente no envía expected_store_id (backwards compat)', async () => {
      sessionsService.countOpenSessions.mockResolvedValue(NO_OPEN_SESSIONS);
