...
export class WompiConfigValidator {
  ...
  @Matches(/^pub_(test|prod)_/, { message: 'La llave pública debe iniciar con pub_test_ o pub_prod_' })
  public_key: string;

  @IsString()
  @IsNotEmpty({ message: 'La llave privada de Wompi es requerida' })
  @Matches(/^prv_(test|prod)_/, { message: 'La llave privada debe iniciar con prv_test_ o prv_prod_' })
  private_key: string;
  ...
  @IsEnum(WompiEnvironmentEnum, { message: 'El ambiente debe ser SANDBOX o PRODUCTION' })
  @IsOptional()
