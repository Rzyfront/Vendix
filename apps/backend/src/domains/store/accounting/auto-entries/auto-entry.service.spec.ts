...
/**
 * Domicilio de restaurante con INC 8 % INCLUIDO (`shipping-inc.util`). Plato
 * base 50.000 + INC 4.000; domicilio 15.000 = base 13.888,89 + INC 1.111,11.
 * El asiento lleva la base del envío a 414505 y TODO el INC (5.111,11) a 2436,
 * por las tres vías: factura validada, venta POS sin factura y venta a crédito.
 */
describe('AutoEntryService — INC del domicilio (restaurante O-33)', () => {
  const codeFor = (key: string): string | null => {
    if (key.endsWith('.shipping_income')) return '414505';
    if (key.endsWith('.inc_payable')) return '2436';
    if (key.endsWith('.iva_payable') || key.endsWith('.vat_payable'))
      return '2408';
    if (key.endsWith('.revenue')) return '4135';
    if (key.endsWith('.accounts_receivable')) return '1305';
    if (key === 'payment.received.cash') return '110505';
    return null;
  };
  ...
  const createService = () => {
    ...
    const accountMapping = {
      getMapping: jest.fn(async (_org: number, key: string) => {
        const account_code = codeFor(key);
        return account_code ? { account_code, source: 'default' } : null;
      }),
    };
    const entryFailure = { recordFailure: jest.fn(), recordSkip: jest.fn() };
    const service = new AutoEntryService(
      prisma as any,
      accountMapping as any,
      { resolveAccountingEntityForFiscal: jest.fn() } as any,
    ...
    const createAutoEntry = jest
      .spyOn(service, 'createAutoEntry')
      .mockResolvedValue({ id: 999 } as any);
    return { service, createAutoEntry, entryFailure };
  };

  const byAccount = (lines: any[]) => {
    const out: Record<string, { debit: number; credit: number }> = {};
    for (const l of lines.filter(Boolean)) {
      const acc = (out[l.account_code] ??= { debit: 0, credit: 0 });
      acc.debit = Math.round((acc.debit + l.debit_amount) * 100) / 100;
      acc.credit = Math.round((acc.credit + l.credit_amount) * 100) / 100;
    }
    return out;
  };
  const expectBalanced = (lines: any[]) => {
    const valid = lines.filter(Boolean);
    const debit = valid.reduce((s: number, l: any) => s + l.debit_amount, 0);
    const credit = valid.reduce((s: number, l: any) => s + l.credit_amount, 0);
    expect(Math.abs(debit - credit)).toBeLessThanOrEqual(0.001);
  };

  // Igual que el emisor: el INC del envío se suma al grupo INC 8 % con su base.
  const breakdown = [
    { tax_type: 'inc' as const, tax_amount: 5111.11, tax_rate: 0.08, taxable_amount: 63888.89 },
  ];

  it('invoice.validated: DR 1305 69.000 = CR 4135 50.000 + 414505 13.888,89 + 2436 5.111,11', async () => {
    ...
    const lines = createAutoEntry.mock.calls[0][0].lines;
    expect(byAccount(lines)).toEqual({
      '1305': { debit: 69000, credit: 0 },
      '4135': { debit: 0, credit: 50000 },
      '414505': { debit: 0, credit: 13888.89 },
      '2436': { debit: 0, credit: 5111.11 },
    });
    expectBalanced(lines);
    // La compuerta F-111 no ve desviación (63.888,89 × 8 % = 5.111,11).
    expect(entryFailure.recordSkip).not.toHaveBeenCalled();
  });

  it('payment.received sin factura: DR caja 69.000 = CR 4135 + 414505 base + 2436 con el INC del envío', async () => {
    const { service, createAutoEntry, entryFailure } = createService();
  ...
  it('credit_sale.created: DR 1305 69.000 = CR 4135 + 414505 base + 2436 con el INC del envío', async () => {
    const { service, createAutoEntry } = createService();
