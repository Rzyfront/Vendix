...
/**
 * GymMemberProfilesService
 *
 * Store-scoped upsert/read for member profiles (`gym_member_profiles`), unique
 * per (store, customer). Holds emergency contact, medical notes, goals and
 * physical metrics — never a raw biometric template.
 *
 * Uses `withoutScope()` + explicit `store_id` (see GymMembershipsService note)
...
export class GymMemberProfilesService {
  ...
  private get profiles() {
    return this.prisma.withoutScope().gym_member_profiles;
  }
