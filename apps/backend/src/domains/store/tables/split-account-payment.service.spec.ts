...
describe('SplitAccountPaymentService', () => {
  ...
  const matches = (row: any, where: any): boolean =>
    Object.entries(where ?? {}).every(([key, value]: [string, any]) => {
      if (key === 'orders')
        return row.order_id === source.id && value.store_id === source.store_id;
  ...
  beforeEach(() => {
    ...
    accounts = [11, 12].map((id) => ({
      ...
      lines: [
        { taxes: [{ tax_type: 'iva', tax_rate: '0.19', tax_amount: '15.20' }] },
      ],
    }));
    methods = [
      'cash',
      ...
      system_payment_method: {
        ...
        is_active: true,
        processing_mode:
          index < 3 ? 'DIRECT' : index === 5 ? 'ON_DELIVERY' : 'ONLINE',
      },
    }));
    payments = [
      {
    ...
    db = {
      ...
      orders: {
        findFirst: jest.fn(async ({ where }) =>
          matches(source, where) ? source : null,
        ),
      ...
      store_payment_methods: {
        findFirst: jest.fn(async ({ where }) =>
          methods.find((m) => matches(m, where)),
        ),
      },
      payments: {
        findFirst: jest.fn(async ({ where, include }) => {
          ...
          return (
            ...
            (include
              ? {
                  ...p,
                  store_payment_method: methods.find(
                    (m) => m.id === p.store_payment_method_id,
                  ),
                }
              : p)
          );
        ...
        findMany: jest.fn(async ({ where }) =>
          payments.filter((p) => matches(p, where)),
        ),
        ...
        updateMany: jest.fn(async ({ where, data }) => {
          const selected = payments.filter((p) => matches(p, where));
          selected.forEach((p) => Object.assign(p, data));
      ...
      cash_register_movements: {
        findFirst: jest.fn(async ({ where }) =>
          movements.find((m) => matches(m, where)),
        ),
        create: jest.fn(async ({ data }) => {
          const m = { id: movements.length + 1, ...data };
          movements.push(m);
          return m;
        }),
      },
    ...
    gateway = {
      processReservedPayment: jest.fn(async (id) => {
        ...
        payment.gateway_response = {
          ...payment.gateway_response,
          nextAction: { type: 'redirect', url: 'https://example.test/pay' },
        };
    ...
    service = new SplitAccountPaymentService(
      db,
      gateway,
      { getSplit: jest.fn(async () => ({ source_order_id: 100 })) } as any,
      events,
      settings,
      sessions,
      tableSessions,
  ...
  it.each([1, 2, 3])(
    'collects manual method %s with canonical event and no gateway/stock/order status change',
    async (store_payment_method_id) => {
      const result = await service.pay(
        100,
        11,
        makeRequest({ store_payment_method_id }),
      );
  ...
  it('rejects same idempotency key with changed amount or reference', async () => {
    ...
    await expect(
      service.pay(100, 11, makeRequest({ amount: 40 })),
    ).rejects.toThrow('clave');
    await expect(
      service.pay(
        100,
        11,
        makeRequest({ amount: 50, payment_reference: 'other' }),
      ),
    ).rejects.toThrow('clave');
  ...
  it('budgets pending and authorized reservations; never reallocates another account capacity', async () => {
    ...
    await expect(
      service.pay(100, 11, makeRequest({ amount: 20.01 })),
    ).rejects.toThrow('saldo');
    await expect(
      service.pay(100, 12, makeRequest({ amount: 100.01 })),
    ).rejects.toThrow('saldo');
  ...
  it('also enforces root budget against money collected elsewhere', async () => {
    ...
    await expect(
      service.pay(100, 11, makeRequest({ amount: 21 })),
    ).rejects.toThrow('saldo');
  });

  it.each([0, -1, 0.001, NaN, Infinity])(
    'rejects invalid amount %s before reserving',
    async (amount) => {
      await expect(
        service.pay(100, 11, makeRequest({ amount })),
  ...
  it('rejects foreign/retained account, insufficient cash, unsupported methods and required closed register', async () => {
    await expect(service.pay(100, 999, makeRequest())).rejects.toBeInstanceOf(
      VendixHttpException,
    );
    ...
    await expect(
      service.pay(100, 11, makeRequest({ amount_received: 90 })),
    ).rejects.toThrow('efectivo');
    await expect(
      service.pay(100, 11, makeRequest({ store_payment_method_id: 5 })),
    ).rejects.toThrow('integrado');
    await expect(
      service.pay(100, 11, makeRequest({ store_payment_method_id: 6 })),
    ).rejects.toThrow('integrado');
    settings.getSettings.mockResolvedValue({
      pos: {
        cash_register: { enabled: true, require_session_for_sales: true },
  ...
  it('validates bank ownership before creating a payment', async () => {
    gateway.resolveAndValidateBankAccount.mockRejectedValue(
      new Error('foreign bank'),
    );
    await expect(
      service.pay(
        100,
        11,
        makeRequest({ store_payment_method_id: 3, bank_account_id: 999 }),
      ),
    ).rejects.toThrow('foreign bank');
    expect(db.payments.create).not.toHaveBeenCalled();
  });

  it('leaves uncertain provider result pending and replay does not repeat external request', async () => {
    gateway.processReservedPayment.mockImplementation(async (id) => {
      payments.find((p) => p.id === id).gateway_reference =
        `financial_10_${id}`;
    ...
    await expect(service.pay(100, 11, request)).rejects.toThrow(
      'provider timeout',
    );
    expect(payments[1].state).toBe('pending');
  ...
  it('keeps Wompi idempotency stable across object-key order and rejects unrelated method fields', async () => {
    await service.pay(
      100,
      11,
      makeRequest({
    ...
    await service.pay(
      ...
      makeRequest({
        store_payment_method_id: 4,
        wompi_payment_method: { phone_number: '3000000000', type: 'NEQUI' },
      }),
    );
    expect(payments).toHaveLength(2);
    expect(gateway.processReservedPayment).toHaveBeenCalledTimes(1);
  ...
  it('retries physical settlement failure without charging or duplicating the payment', async () => {
    ...
    await service.reconcileReceivedForOrder(100);
    expect(payments[2].financial_effects_recorded_at).toBeInstanceOf(Date);
    expect(payments).toHaveLength(3);
    expect(gateway.processReservedPayment).not.toHaveBeenCalled();
    expect(orderFlow.settleFinancialSplitSource).toHaveBeenCalledTimes(2);
  });

  it('maps simultaneous cross-source idempotency collision to canonical 409', async () => {
    db.$transaction.mockRejectedValueOnce(
      new Prisma.PrismaClientKnownRequestError('unique', {
        code: 'P2002',
        clientVersion: 'test',
        meta: { target: ['financial_idempotency_key'] },
      }),
    );
