...
export class MembershipAccessService {
  ...
  async validate(dto: ValidateAccessDto): Promise<AccessValidationResult> {
    ...
    // 3. Optional per-period access quota — the cap lives in the plan's
    //    `features` Json (`features.access_limit_per_period`), not a column.
    const plan = await this.membershipPlans.findFirst({
      where: { id: active.plan_id, store_id: storeId },
      select: { features: true },
    });
    const rawLimit = (
      plan?.features as Record<string, any> | null | undefined
  ...
  private async logAndReturn(
    ...
    return {
      granted: entry.result === membership_access_result_enum.granted,
      result: entry.result,
      reason: entry.reason,
      customer_id: entry.customer_id,
      membership_id: entry.membership_id,
