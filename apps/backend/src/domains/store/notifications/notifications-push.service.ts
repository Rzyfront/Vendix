...
export class NotificationsPushService implements OnModuleInit {
   ...
   */
  onModuleInit() {
    this.vapid_public_key = (process.env.VAPID_PUBLIC_KEY || '').replace(
      /\s/g,
      '',
    );
    ...
    const vapid_subject = (
      process.env.VAPID_SUBJECT || 'mailto:support@vendix.online'
    ).trim();
  ...
  async sendToStore(
    ...
    try {
      ...
      const active_subs =
        await this.global_prisma.notification_subscriptions.findMany({
