...
// grafo de AppModule en un worker construye el árbol completo de módulos y
...
describeDestructiveE2E('Owner Registration Flow - Integration Tests', () => {
  ...
  describe('Complete Owner Registration Flow', () => {
    ...
    it('should handle email verification with expired token', async () => {
      ...
      // Get the user and create an expired token
      const user = await prismaService.users.findFirst({
        where: { email: validOwnerData.email },
      });
    ...
    it('should handle resend verification for unverified email', async () => {
      ...
      // Get the user and invalidate existing tokens
      const user = await prismaService.users.findFirst({
        where: { email: validOwnerData.email },
      });
    ...
    it('should handle pending onboarding scenario', async () => {
      // Create an organization with onboarding incomplete
      const existingOrg = await prismaService.organizations.create({
        data: {
      ...
      // Create existing user with owner role
      ...
      const existingUser = await prismaService.users.findFirst({
        where: { email: validOwnerData.email },
      });
      ...
      expect(response.body.success).toBe(false);
      expect(response.body.message).toContain('onboarding pendiente');
    });
  });
  ...
  describe('Security and Edge Cases', () => {
    ...
    it('should handle concurrent registration attempts gracefully', async () => {
      // Attempt to register the same user twice concurrently
      const promises = [
      ...
      expect(failCount).toBe(1); // One should fail
    });
    ...
    it('should handle special characters in organization name', async () => {
      const specialName = 'Test Organization!@#$%^&*()';

      // This should work (slug generation will handle special chars)
      const response = await request(app.getHttpServer())
        .post('/auth/register-owner')
        .send({
          ...validOwnerData,
          organization_name: specialName,
          email: 'special@test.com', // Different email to avoid conflicts
        })
        .expect(201);
