import {
  Component,
  Input,
  Output,
  EventEmitter,
  OnInit,
  OnChanges,
  SimpleChanges,
} from '@angular/core';
import { CommonModule } from '@angular/common';
import {
  FormBuilder,
  FormGroup,
  ReactiveFormsModule,
  Validators,
} from '@angular/forms';

import { InputComponent } from '../../input/input.component';
import { ButtonComponent } from '../../button/button.component';
import { IconComponent } from '../../icon/icon.component';
import { SelectorComponent } from '../../selector/selector.component';
import { ToggleComponent } from '../../toggle/toggle.component';

import {
  StoreConfigData,
  OrganizationConfigData,
} from '../interfaces/onboarding.interface';

@Component({
  selector: 'app-store-config-step',
  standalone: true,
  imports: [
    CommonModule,
    ReactiveFormsModule,
    InputComponent,
    IconComponent,
    SelectorComponent,
    ToggleComponent,
  ],
  template: `
    <div class="space-y-6">
      <!-- Header del Paso -->
      <div class="text-center">
        <div
          class="inline-flex items-center justify-center w-16 h-16 bg-[var(--color-primary)]/10 rounded-full mb-4"
        >
          <app-icon
            name="store"
            [size]="32"
            class="text-[var(--color-primary)]"
          ></app-icon>
        </div>
        <h3
          class="text-[var(--fs-lg)] font-[var(--fw-semibold)] text-[var(--color-text-primary)] mb-2"
        >
          Configura tu Tienda
        </h3>
        <p
          class="text-[var(--fs-sm)] text-[var(--color-text-secondary)] max-w-md mx-auto"
        >
          Define los datos de tu punto de venta o tienda online
        </p>
      </div>

      <!-- Autocompletado desde Organización -->
      <div
        *ngIf="hasOrganizationData"
        class="bg-[var(--color-primary)]/5 border border-[var(--color-primary)]/20 rounded-[var(--radius-md)] p-4"
      >
        <div class="flex items-start gap-3">
          <app-icon
            name="sparkles"
            [size]="16"
            class="text-[var(--color-primary)] mt-0.5"
          ></app-icon>
          <div class="flex-1">
            <h4
              class="text-[var(--fs-sm)] font-medium text-[var(--color-text-primary)] mb-1"
            >
              Información autocompletada
            </h4>
            <p class="text-[var(--fs-xs)] text-[var(--color-text-secondary)]">
              Hemos prellenado algunos campos con la información de tu
              organización. Puedes modificarlos si lo necesitas.
            </p>
          </div>
        </div>
      </div>

      <!-- Formulario -->
      <form [formGroup]="form" class="space-y-4">
        <!-- Nombre de la Tienda -->
        <div>
          <label
            class="block text-[var(--fs-sm)] font-medium text-[var(--color-text-primary)] mb-2"
          >
            Nombre de la Tienda *
          </label>
          <app-input
            formControlName="name"
            placeholder="Tienda Principal"
            [size]="'md'"
            [error]="getErrorMessage('name')"
          ></app-input>
        </div>

        <!-- Descripción -->
        <div>
          <label
            class="block text-[var(--fs-sm)] font-medium text-[var(--color-text-primary)] mb-2"
          >
            Descripción
          </label>
          <textarea
            formControlName="description"
            rows="3"
            class="w-full px-3 py-2 border border-[var(--color-border)] rounded-[var(--radius-md)] focus:outline-none focus:ring-2 focus:ring-[var(--color-ring)] focus:border-[var(--color-primary)] bg-[var(--color-surface)] text-[var(--color-text-primary)] placeholder-[var(--color-text-muted)] resize-none"
            placeholder="Describe brevemente tu tienda..."
          ></textarea>
        </div>

        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <!-- Tipo de Tienda -->
          <div>
            <label
              class="block text-[var(--fs-sm)] font-medium text-[var(--color-text-primary)] mb-2"
            >
              Tipo de Tienda *
            </label>
            <app-selector
              formControlName="store_type"
              [placeholder]="'Selecciona tipo de tienda'"
              [options]="storeTypeOptions"
              [size]="'md'"
            ></app-selector>
          </div>

          <!-- Email de Contacto -->
          <div>
            <label
              class="block text-[var(--fs-sm)] font-medium text-[var(--color-text-primary)] mb-2"
            >
              Email de Contacto
            </label>
            <app-input
              formControlName="email"
              placeholder="tienda@empresa.com"
              [size]="'md'"
              [error]="getErrorMessage('email')"
            ></app-input>
          </div>
        </div>

        <!-- Teléfono -->
        <div>
          <label
            class="block text-[var(--fs-sm)] font-medium text-[var(--color-text-primary)] mb-2"
          >
            Teléfono de la Tienda
          </label>
          <app-input
            formControlName="phone"
            placeholder="+1 (555) 123-4567"
            [size]="'md'"
            [error]="getErrorMessage('phone')"
          ></app-input>
        </div>

        <!-- Configuración Regional -->
        <div class="border-t border-[var(--color-border)] pt-4">
          <h4
            class="text-[var(--fs-md)] font-medium text-[var(--color-text-primary)] mb-4"
          >
            Configuración Regional
          </h4>

          <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <!-- Zona Horaria -->
            <div>
              <label
                class="block text-[var(--fs-sm)] font-medium text-[var(--color-text-primary)] mb-2"
              >
                Zona Horaria
              </label>
              <app-selector
                formControlName="timezone"
                [placeholder]="'Selecciona zona horaria'"
                [options]="timezoneOptions"
                [size]="'md'"
              ></app-selector>
            </div>

            <!-- Moneda -->
            <div>
              <label
                class="block text-[var(--fs-sm)] font-medium text-[var(--color-text-primary)] mb-2"
              >
                Moneda
              </label>
              <app-selector
                formControlName="currency"
                [placeholder]="'Selecciona moneda'"
                [options]="currencyOptions"
                [size]="'md'"
              ></app-selector>
            </div>

            <!-- Idioma -->
            <div>
              <label
                class="block text-[var(--fs-sm)] font-medium text-[var(--color-text-primary)] mb-2"
              >
                Idioma
              </label>
              <app-selector
                formControlName="language"
                [placeholder]="'Selecciona idioma'"
                [options]="languageOptions"
                [size]="'md'"
              ></app-selector>
            </div>
          </div>
        </div>

        <!-- Configuración de Funcionalidades -->
        <div class="border-t border-[var(--color-border)] pt-4">
          <h4
            class="text-[var(--fs-md)] font-medium text-[var(--color-text-primary)] mb-4"
          >
            Funcionalidades de la Tienda
          </h4>

          <div class="space-y-3">
            <!-- Control de Inventario -->
            <div
              class="flex items-center justify-between p-3 bg-[var(--color-surface)] border border-[var(--color-border)] rounded-[var(--radius-md)]"
            >
              <div class="flex-1">
                <h5
                  class="text-[var(--fs-sm)] font-medium text-[var(--color-text-primary)]"
                >
                  Control de Inventario
                </h5>
                <p
                  class="text-[var(--fs-xs)] text-[var(--color-text-secondary)] mt-1"
                >
                  Gestiona automáticamente el stock de tus productos
                </p>
              </div>
              <app-toggle
                formControlName="inventory_tracking"
                (changed)="onToggleChange('inventory_tracking', $event)"
              ></app-toggle>
            </div>

            <!-- Cálculo de Impuestos -->
            <div
              class="flex items-center justify-between p-3 bg-[var(--color-surface)] border border-[var(--color-border)] rounded-[var(--radius-md)]"
            >
              <div class="flex-1">
                <h5
                  class="text-[var(--fs-sm)] font-medium text-[var(--color-text-primary)]"
                >
                  Cálculo de Impuestos
                </h5>
                <p
                  class="text-[var(--fs-xs)] text-[var(--color-text-secondary)] mt-1"
                >
                  Calcula automáticamente los impuestos en tus ventas
                </p>
              </div>
              <app-toggle
                formControlName="tax_calculation"
                (changed)="onToggleChange('tax_calculation', $event)"
              ></app-toggle>
            </div>
          </div>
        </div>

        <!-- Horario de Atención -->
        <div class="border-t border-[var(--color-border)] pt-4">
          <h4
            class="text-[var(--fs-md)] font-medium text-[var(--color-text-primary)] mb-4"
          >
            Horario de Atención
          </h4>

          <div
            formGroupName="business_hours"
            class="grid grid-cols-1 md:grid-cols-2 gap-3"
          >
            <div
              *ngFor="let day of weekDays; let i = index"
              [formGroupName]="day.key"
              class="flex items-center gap-3 p-3 bg-[var(--color-surface)] border border-[var(--color-border)] rounded-[var(--radius-md)]"
            >
              <div class="flex-1">
                <label
                  class="text-[var(--fs-sm)] font-medium text-[var(--color-text-primary)]"
                >
                  {{ day.label }}
                </label>
                <div
                  *ngIf="!isDayClosed(day.key)"
                  class="flex items-center gap-2 mt-2"
                >
                  <input
                    type="time"
                    formControlName="open"
                    class="px-2 py-1 text-sm border border-[var(--color-border)] rounded focus:outline-none focus:ring-1 focus:ring-[var(--color-ring)]"
                  />
                  <span
                    class="text-[var(--fs-xs)] text-[var(--color-text-muted)]"
                    >a</span
                  >
                  <input
                    type="time"
                    formControlName="close"
                    class="px-2 py-1 text-sm border border-[var(--color-border)] rounded focus:outline-none focus:ring-1 focus:ring-[var(--color-ring)]"
                  />
                </div>
              </div>
              <app-toggle
                [checked]="!isDayClosed(day.key)"
                (changed)="onDayToggle(day.key, $event)"
              ></app-toggle>
            </div>
          </div>
        </div>
      </form>

      <!-- Información Adicional -->
      <div
        class="bg-[var(--color-surface)] border border-[var(--color-border)] rounded-[var(--radius-md)] p-4"
      >
        <div class="flex items-start gap-3">
          <app-icon
            name="info"
            [size]="16"
            class="text-[var(--color-primary)] mt-0.5"
          ></app-icon>
          <div class="flex-1">
            <h4
              class="text-[var(--fs-sm)] font-medium text-[var(--color-text-primary)] mb-1"
            >
              Configuración de tienda
            </h4>
            <p class="text-[var(--fs-xs)] text-[var(--color-text-secondary)]">
              Estos datos serán utilizados para configurar tu dominio y acceso
              en el siguiente paso. Podrás modificarlos más tarde en la
              configuración de tu tienda.
            </p>
          </div>
        </div>
      </div>
    </div>
  `,
  styles: [
    `
      :host {
        display: block;
      }

      textarea {
        font-family: inherit;
        font-size: var(--fs-sm);
        line-height: 1.5;
      }

      textarea:focus {
        outline: none;
        box-shadow: 0 0 0 2px var(--color-ring);
      }

      input[type='time'] {
        font-family: inherit;
      }
    `,
  ],
})
export class StoreConfigStepComponent implements OnInit, OnChanges {
  @Input() form: FormGroup;
  @Input() data: StoreConfigData = {
    name: '',
    description: '',
    store_type: 'retail',
    phone: '',
    email: '',
    settings: {
      timezone: 'America/New_York',
      currency: 'USD',
      language: 'es',
      inventory_tracking: true,
      tax_calculation: true,
    },
    business_hours: {
      monday: { open: '09:00', close: '18:00' },
      tuesday: { open: '09:00', close: '18:00' },
      wednesday: { open: '09:00', close: '18:00' },
      thursday: { open: '09:00', close: '18:00' },
      friday: { open: '09:00', close: '18:00' },
      saturday: { open: '10:00', close: '16:00' },
      sunday: { closed: true },
    },
  };
  @Input() organizationData: OrganizationConfigData = {
    name: '',
    description: '',
    industry: '',
    website: '',
    phone: '',
    settings: {
      timezone: 'America/New_York',
      currency: 'USD',
      language: 'es',
      date_format: 'DD/MM/YYYY',
    },
    billing: {
      tax_id: '',
      billing_email: '',
    },
  };
  @Output() dataChange = new EventEmitter<StoreConfigData>();
  @Output() validityChange = new EventEmitter<boolean>();

  // Opciones para los selectores
  storeTypeOptions = [
    { value: 'retail', label: 'Tienda Física' },
    { value: 'online', label: 'Tienda Online' },
    { value: 'hybrid', label: 'Híbrida (Física + Online)' },
  ];

  timezoneOptions = [
    { value: 'America/New_York', label: 'Nueva York (EST)' },
    { value: 'America/Chicago', label: 'Chicago (CST)' },
    { value: 'America/Denver', label: 'Denver (MST)' },
    { value: 'America/Los_Angeles', label: 'Los Ángeles (PST)' },
    { value: 'Europe/Madrid', label: 'Madrid (CET)' },
    { value: 'Europe/London', label: 'Londres (GMT)' },
    { value: 'Asia/Tokyo', label: 'Tokio (JST)' },
  ];

  currencyOptions = [
    { value: 'USD', label: 'USD - Dólar Americano' },
    { value: 'EUR', label: 'EUR - Euro' },
    { value: 'GBP', label: 'GBP - Libra Esterlina' },
    { value: 'MXN', label: 'MXN - Peso Mexicano' },
    { value: 'ARS', label: 'ARS - Peso Argentino' },
    { value: 'COP', label: 'COP - Peso Colombiano' },
  ];

  languageOptions = [
    { value: 'es', label: 'Español' },
    { value: 'en', label: 'English' },
    { value: 'pt', label: 'Português' },
  ];

  weekDays = [
    { key: 'monday', label: 'Lunes' },
    { key: 'tuesday', label: 'Martes' },
    { key: 'wednesday', label: 'Miércoles' },
    { key: 'thursday', label: 'Jueves' },
    { key: 'friday', label: 'Viernes' },
    { key: 'saturday', label: 'Sábado' },
    { key: 'sunday', label: 'Domingo' },
  ];

  get hasOrganizationData(): boolean {
    return !!(this.organizationData?.name || this.organizationData?.phone);
  }

  constructor(private fb: FormBuilder) {
    this.form = this.fb.group({});
  }

  ngOnInit(): void {
    this.initializeForm();
    this.setupFormListeners();
  }

  ngOnChanges(changes: SimpleChanges): void {
    if (changes['data'] && changes['data'].currentValue) {
      this.updateFormData();
    }
  }

  private initializeForm(): void {
    // Create business hours group first
    const businessHoursGroup: { [key: string]: any } = {};

    this.weekDays.forEach((day) => {
      if (this.isDayClosed(day.key)) {
        businessHoursGroup[day.key] = this.fb.group({
          closed: [true],
        });
      } else {
        const dayHours =
          this.data?.business_hours?.[
            day.key as keyof typeof this.data.business_hours
          ];
        const openTime = (dayHours as any)?.open || '09:00';
        const closeTime = (dayHours as any)?.close || '18:00';
        businessHoursGroup[day.key] = this.fb.group({
          open: [openTime],
          close: [closeTime],
          closed: [false],
        });
      }
    });

    this.form = this.fb.group({
      name: [
        this.data?.name || '',
        [
          Validators.required,
          Validators.minLength(2),
          Validators.maxLength(100),
        ],
      ],
      description: [this.data?.description || '', [Validators.maxLength(500)]],
      store_type: [this.data?.store_type || 'retail', [Validators.required]],
      email: [this.data?.email || '', [Validators.email]],
      phone: [
        this.data?.phone || '',
        [
          Validators.pattern(
            /^[\+]?[(]?[0-9]{1,4}[)]?[-\s\.]?[(]?[0-9]{1,4}[)]?[-\s\.]?[0-9]{1,9}$/,
          ),
        ],
      ],
      timezone: [
        this.data?.settings?.timezone || 'America/New_York',
        [Validators.required],
      ],
      currency: [this.data?.settings?.currency || 'USD', [Validators.required]],
      language: [this.data?.settings?.language || 'es', [Validators.required]],
      inventory_tracking: [
        this.data?.settings?.inventory_tracking !== undefined
          ? this.data?.settings.inventory_tracking
          : true,
        [],
      ],
      tax_calculation: [
        this.data?.settings?.tax_calculation !== undefined
          ? this.data?.settings.tax_calculation
          : true,
        [],
      ],
      business_hours: this.fb.group(businessHoursGroup),
    });
  }

  private setupFormListeners(): void {
    this.form.valueChanges.subscribe((values) => {
      const businessHours: any = {};

      if (values.business_hours) {
        this.weekDays.forEach((day) => {
          const dayHours = values.business_hours[day.key];
          if (dayHours?.closed) {
            businessHours[day.key] = { closed: true };
          } else {
            businessHours[day.key] = {
              open: dayHours?.open || '09:00',
              close: dayHours?.close || '18:00',
            };
          }
        });
      }

      const storeData: StoreConfigData = {
        ...this.data,
        name: values.name,
        description: values.description,
        store_type: values.store_type,
        email: values.email,
        phone: values.phone,
        settings: {
          timezone: values.timezone,
          currency: values.currency,
          language: values.language,
          inventory_tracking: values.inventory_tracking,
          tax_calculation: values.tax_calculation,
        },
        business_hours: businessHours,
      };

      this.dataChange.emit(storeData);
      this.validityChange.emit(this.form.valid);
    });
  }

  private updateFormData(): void {
    if (this.form && this.data) {
      const businessHoursPatch: any = {};

      this.weekDays.forEach((day) => {
        const hours =
          this.data.business_hours?.[
            day.key as keyof typeof this.data.business_hours
          ];
        if (hours) {
          businessHoursPatch[day.key] = hours;
        }
      });

      this.form.patchValue(
        {
          name: this.data.name,
          description: this.data.description,
          store_type: this.data.store_type,
          email: this.data.email,
          phone: this.data.phone,
          timezone: this.data.settings?.timezone,
          currency: this.data.settings?.currency,
          language: this.data.settings?.language,
          inventory_tracking: this.data.settings?.inventory_tracking,
          tax_calculation: this.data.settings?.tax_calculation,
          business_hours: businessHoursPatch,
        },
        { emitEvent: false },
      );
    }
  }

  isDayClosed(dayKey: string): boolean {
    const hours =
      this.data?.business_hours?.[
        dayKey as keyof typeof this.data.business_hours
      ];
    return hours && 'closed' in hours && hours.closed;
  }

  onDayToggle(dayKey: string, isOpen: boolean): void {
    const currentBusinessHours = { ...this.data.business_hours };

    if (isOpen) {
      (currentBusinessHours as any)[dayKey] = { open: '09:00', close: '18:00' };
    } else {
      (currentBusinessHours as any)[dayKey] = { closed: true };
    }

    this.dataChange.emit({
      ...this.data,
      business_hours: currentBusinessHours,
    });
  }

  onToggleChange(field: string, value: boolean): void {
    // El cambio se manejará a través del form.valueChanges
  }

  getErrorMessage(fieldName: string): string {
    const field = this.form.get(fieldName);
    if (!field || !field.errors) return '';

    const errors = field.errors;

    if (errors['required']) {
      return 'Este campo es requerido';
    }

    if (errors['minlength']) {
      return `Mínimo ${errors['minlength'].requiredLength} caracteres`;
    }

    if (errors['maxlength']) {
      return `Máximo ${errors['maxlength'].requiredLength} caracteres`;
    }

    if (errors['email']) {
      return 'Email inválido';
    }

    if (errors['pattern']) {
      if (fieldName === 'phone') {
        return 'Formato de teléfono inválido';
      }
      return 'Formato inválido';
    }

    return 'Campo inválido';
  }
}
