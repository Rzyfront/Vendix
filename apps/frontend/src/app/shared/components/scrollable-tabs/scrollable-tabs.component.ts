...
export class ScrollableTabsComponent implements AfterViewInit {
  ...
  readonly scroll_container =
    viewChild.required<ElementRef<HTMLElement>>('scroll_container');
  ...
  readonly tab_icon_size = computed(() => {
    const map: Record<ScrollableTabSize, number> = {
      xs: 10,
      sm: 12,
      md: 12,
      lg: 14,
  ...
  private scrollActiveTabIntoView(tab_id: string): void {
    ...
    const tab_el = container.querySelector(
      `[data-tab-id="${tab_id}"]`,
    ) as HTMLElement;
    if (tab_el) {
      tab_el.scrollIntoView({
        behavior: 'smooth',
        block: 'nearest',
        inline: 'nearest',
      });
