...
import {
  ...
  OnboardingFiscalData,
} from '../../../core/services/onboarding-wizard.service';
import { AuthFacade } from '../../../core/store/auth/auth.facade';
import { ToastService } from '../toast/toast.service';
import { EnvironmentSwitchService } from '../../../core/services/environment-switch.service';
import { parseApiError } from '../../../core/utils/parse-api-error';
import { OnboardingFiscalValue } from './components/onboarding-fiscal-section/onboarding-fiscal-section.component';
            ...
            @if (businessType() === 'STORE' && currentStep() > 3) {
              ...
              @if (currentStep() === 4) {
                <app-store-setup-step
                  ...
                  [fiscalInitialValue]="storeFiscalInitialValue()"
                  (fiscalValueChange)="onStoreFiscalChange($event)"
                  (fiscalValidityChange)="storeFiscalValid.set($event)"
                  (previousStep)="previousStep()"
                ></app-store-setup-step>
              }
            ...
            @if (businessType() === 'ORGANIZATION' && currentStep() > 3) {
              ...
              @if (currentStep() === 4) {
                <app-organization-setup-step
                  ...
                  [businessType]="businessType()"
                  [fiscalInitialValue]="orgFiscalInitialValue()"
                  (fiscalValueChange)="onOrgFiscalChange($event)"
                  (fiscalValidityChange)="orgFiscalValid.set($event)"
                  (previousStep)="previousStep()"
                ></app-organization-setup-step>
              }
...
export class OnboardingModalComponent {
  ...
  // --- Fiscal section state (optional, captured from the reusable component) ---
  readonly orgFiscalValue = signal<Partial<OnboardingFiscalValue> | null>(null);
  readonly orgFiscalValid = signal(true);
  readonly storeFiscalValue = signal<Partial<OnboardingFiscalValue> | null>(
    null,
  );
  readonly storeFiscalValid = signal(true);

  // Prefill the org fiscal section from the user's organization data, if any.
  readonly orgFiscalInitialValue = signal<Partial<OnboardingFiscalValue> | null>(
    null,
  );
  readonly storeFiscalInitialValue =
    signal<Partial<OnboardingFiscalValue> | null>(null);
  ...
  onBusinessTypeSelected(event: {
    ...
    // Fiscal scope is derived from the business type only: STORE -> STORE,
    // ORGANIZATION -> consolidated ORGANIZATION. ORGANIZATION + STORE is no
    // longer offered in onboarding.
    const fiscalScope = event.type === 'STORE' ? 'STORE' : 'ORGANIZATION';
  ...
  onOrgFiscalChange(value: Partial<OnboardingFiscalValue>): void {
    this.orgFiscalValue.set(value);
  }

  onStoreFiscalChange(value: Partial<OnboardingFiscalValue>): void {
    this.storeFiscalValue.set(value);
  }

  /**
   * Build the optional `fiscal_data` payload from the captured fiscal value.
   * Returns `undefined` when the user left every field empty so we never send
   * an empty fiscal object to the backend.
   */
  private buildFiscalData(
    value: Partial<OnboardingFiscalValue> | null,
  ): OnboardingFiscalData | undefined {
    if (!value) return undefined;

    const fiscalData: OnboardingFiscalData = {};
    if (value.legal_name?.trim()) fiscalData.legal_name = value.legal_name.trim();
    if (value.nit?.trim()) fiscalData.nit = value.nit.trim();
    if (value.nit_dv?.trim()) fiscalData.nit_dv = value.nit_dv.trim();

    // Type and regime are only meaningful when an identifier was provided.
    if (fiscalData.nit) {
      if (value.nit_type) fiscalData.nit_type = value.nit_type;
      if (value.tax_regime) fiscalData.tax_regime = value.tax_regime;
    }

    return Object.keys(fiscalData).length > 0 ? fiscalData : undefined;
  }
  ...
  private initializeForms(): void {
    ...
    this.organizationForm = this.fb.group({
      ...
      description: [''],
      // Fiscal/legal data (legal_name, NIT, tax_regime…) is now captured by the
      // optional `app-onboarding-fiscal-section` and sent as `fiscal_data`.
      ...
      .subscribe((user) => {
        if (user) {
          ...
          if (user.organizations?.name) {
            ...
            // Seed the optional fiscal section with any known legal data.
            const legalName = user.organizations.legal_name;
            if (legalName) {
              this.orgFiscalInitialValue.set({ legal_name: legalName });
            }
          }
        }
      });
  ...
  private submitStoreSetup(): void {
    ...
    // Fiscal data only applies to the single-store (STORE) flow.
    const isStoreFlow = this.businessType() === 'STORE';

    if (isStoreFlow && !this.storeFiscalValid()) {
      this.toastService.warning(
        'Revisa los datos fiscales: el dígito de verificación no coincide con el NIT.',
        'Información fiscal',
      );
      this.isProcessing.set(false);
      return;
    ...
    this.isSubmitting.set(true);
    const fiscalData = isStoreFlow
      ? this.buildFiscalData(this.storeFiscalValue())
      : undefined;
    const storeData = {
      ...this.storeForm.value,
      ...(fiscalData ? { fiscal_data: fiscalData } : {}),
    };
  ...
  private submitOrganizationSetup(): void {
    ...
    // Block submission only when the user actually typed an inconsistent NIT/DV.
    if (!this.orgFiscalValid()) {
      this.toastService.warning(
        'Revisa los datos fiscales: el dígito de verificación no coincide con el NIT.',
        'Información fiscal',
      );
    ...
    this.isSubmitting.set(true);
    const fiscalData = this.buildFiscalData(this.orgFiscalValue());
  ...
  private handleOnboardingError(error: any, defaultMessage: string): void {
    ...
    // Standardized parse: surfaces a safe UX message and known error codes.
    const parsed = parseApiError(error);

    // Fiscal validation errors: keep the user in the wizard with an actionable
    // toast instead of an opaque "configuration failed" message.
    const fiscalSignal = `${parsed.errorCode ?? ''} ${parsed.devMessage ?? ''}`.toLowerCase();
    if (
      fiscalSignal.includes('fiscal') ||
      fiscalSignal.includes('nit') ||
      fiscalSignal.includes('dian')
    ) {
      this.toastService.warning(
        'Revisa los datos fiscales (NIT, dígito de verificación o régimen) e inténtalo de nuevo. Recuerda que son opcionales y puedes dejarlos vacíos.',
        'Información fiscal',
        6000,
      );
      return;
    ...
    if (error?.error?.message) {
      ...
      // No raw message but we have a mapped UX message from the error code.
      displayMessage = parsed.userMessage;
    }
