...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, FormsModule, RouterModule, IconComponent, ConfirmationModalComponent],
  template: `
    <section class="fiscal-page">
      <header class="fiscal-header">
        <div>
          <p class="fiscal-kicker">Configuración</p>
          <h1>Manejo fiscal</h1>
        </div>
        <div class="header-actions">
          @if (service.lastStatus()?.store_statuses?.length) {
            <select
              [ngModel]="service.targetStoreId()"
              (ngModelChange)="selectStore($event)"
              aria-label="Tienda fiscal"
            >
              @for (store of service.lastStatus()?.store_statuses || []; track store.store_id) {
                <option [ngValue]="store.store_id">{{ store.store_name }}</option>
              }
            </select>
          }
          <button class="secondary-btn" type="button" (click)="refresh()" [disabled]="service.loading()">
