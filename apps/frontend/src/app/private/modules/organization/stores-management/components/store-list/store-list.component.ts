...
@Component({
  ...
  template: `
    <div class="bg-white rounded-[var(--radius-lg)] shadow-[var(--shadow-md)] overflow-hidden">
      <!-- Table Header -->
        ...
        <div class="flex items-center justify-between">
          ...
          <div class="flex items-center space-x-2">
            <span class="text-sm text-[var(--color-text-secondary)]">
              Mostrando {{ stores.length || 0 }} de {{ pagination?.total || 0 }} tiendas
            </span>
          </div>
        </div>
      </div>
      ...
      <!-- Empty State -->
      <div *ngIf="!stores || stores.length === 0" class="px-6 py-12 text-center">
        <app-card class="max-w-md mx-auto text-center p-8">
          <app-icon name="building" [size]="48" color="var(--color-text-tertiary)" class="mb-4" />
          <h3 class="text-lg font-semibold text-[var(--color-text-primary)] mb-2">
            No se encontraron tiendas
          </h3>
          <p class="text-[var(--color-text-secondary)] mb-6">
            Comienza creando tu primera tienda para administrar tu inventario y ventas
          </p>
      ...
      <div *ngIf="stores && stores.length > 0" class="overflow-x-auto">
        <table class="min-w-full divide-y divide-border">
          <thead class="bg-[var(--color-background)]">
            <tr>
              <th class="px-6 py-3 text-left text-xs font-medium text-[var(--color-text-secondary)] uppercase tracking-wider cursor-pointer"
                  (click)="sortBy('name')">
                <div class="flex items-center space-x-1">
                  <span>Nombre</span>
                  <app-icon *ngIf="sortColumn === 'name'" name="chevron-up" [size]="16"
              ...
              <th class="px-6 py-3 text-left text-xs font-medium text-[var(--color-text-secondary)] uppercase tracking-wider cursor-pointer"
                ...
                <div class="flex items-center space-x-1">
                  <span>Tipo</span>
                  <app-icon *ngIf="sortColumn === 'store_type'" name="chevron-up" [size]="16"
                           [style.transform]="sortDirection === 'desc' ? 'rotate(180deg)' : 'none'" />
                </div>
              </th>
              <th class="px-6 py-3 text-left text-xs font-medium text-[var(--color-text-secondary)] uppercase tracking-wider">
              ...
              <th class="px-6 py-3 text-left text-xs font-medium text-[var(--color-text-secondary)] uppercase tracking-wider cursor-pointer"
                ...
                <div class="flex items-center space-x-1">
                  <span>Estado</span>
                  <app-icon *ngIf="sortColumn === 'is_active'" name="chevron-up" [size]="16"
                           [style.transform]="sortDirection === 'desc' ? 'rotate(180deg)' : 'none'" />
                </div>
              </th>
              <th class="px-6 py-3 text-left text-xs font-medium text-[var(--color-text-secondary)] uppercase tracking-wider">
              ...
              <th class="px-6 py-3 text-right text-xs font-medium text-[var(--color-text-secondary)] uppercase tracking-wider">
                Acciones
              </th>
            </tr>
          </thead>
          <tbody class="bg-white divide-y divide-border">
            <tr *ngFor="let store of stores" class="hover:bg-[var(--color-background)]">
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="flex items-center">
                  <div class="flex-shrink-0 h-10 w-10">
                    <div *ngIf="store.logo_url" class="h-10 w-10 rounded-full overflow-hidden">
                      <img [src]="store.logo_url" [alt]="store.name" class="h-full w-full object-cover">
                    </div>
                    <div *ngIf="!store.logo_url" class="h-10 w-10 rounded-full bg-gray-200 flex items-center justify-center">
              ...
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                <div *ngIf="store.addresses && store.addresses.length > 0">
                  {{ store.addresses[0].city }}, {{ store.addresses[0].country_code }}
                </div>
                <div *ngIf="!store.addresses || store.addresses.length === 0" class="text-gray-400">
                  Sin dirección
                </div>
              </td>
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
              ...
              <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                <div class="flex items-center justify-end space-x-2">
                  <button (click)="viewStore.emit(store)"
                  ...
                  <button (click)="editStore.emit(store)"
                  ...
                  <button (click)="toggleStoreStatus(store)"
                          [class]="store.is_active ? 'text-orange-600 hover:text-orange-900' : 'text-green-600 hover:text-green-900'"
                          [title]="store.is_active ? 'Desactivar tienda' : 'Activar tienda'">
                    <svg *ngIf="store.is_active" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <button (click)="deleteStore.emit(store)"
      ...
      <!-- Pagination -->
      <div *ngIf="pagination && pagination.total_pages > 1"
        ...
        <div class="flex items-center justify-between">
          ...
          <div class="flex items-center space-x-2">
            <button (click)="pageChange.emit(pagination.page - 1)"
