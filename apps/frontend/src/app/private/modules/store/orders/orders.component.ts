import { Component } from '@angular/core';
import { CommonModule } from '@angular/common';
import { OrderStatsComponent } from './components/order-stats';

@Component({
  selector: 'app-orders',
  standalone: true,
  imports: [CommonModule, OrderStatsComponent],
  template: `
    <div class="p-6">
      <div class="mb-6">
        <h1 class="text-3xl font-bold text-gray-900 mb-2">Orders Management</h1>
        <p class="text-gray-600">
          Manage customer orders, fulfillment, and tracking
        </p>
      </div>

      <!-- Order Statistics Component -->
      <app-order-stats></app-order-stats>

      <div class="bg-white rounded-lg shadow-sm border p-8 mt-8">
        <div class="text-center">
          <div
            class="inline-flex items-center justify-center w-16 h-16 bg-blue-100 rounded-full mb-4"
          >
            <svg
              class="w-8 h-8 text-blue-600"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
              <path
                stroke-linecap="round"
                stroke-linejoin="round"
                stroke-width="2"
                d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01"
              ></path>
            </svg>
          </div>
          <h2 class="text-xl font-semibold text-gray-900 mb-2">Orders List</h2>
          <p class="text-gray-600 max-w-md mx-auto">
            View and manage all customer orders with detailed information and
            status tracking.
          </p>
        </div>
      </div>
    </div>
  `,
  styles: [
    `
      :host {
        display: block;
        width: 100%;
      }
    `,
  ],
})
export class OrdersComponent {}
