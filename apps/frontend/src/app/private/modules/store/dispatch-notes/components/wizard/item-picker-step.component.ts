...
/**
 * Item-picker step (step 2 for non-customer_delivery subtypes — ref R4a / R4c).
 *
 * Three capture modes:
 *  - **Manual** (always): free product search + add (unchanged behaviour).
  ...
  template: `
    <div class="space-y-2">
      ...
      @if (effectiveMode() === 'excel') {
        ...
        <div
          class="rounded-lg p-2.5 text-[11px] leading-relaxed"
      ...
      @if (importWarnings().length > 0) {
        <div
          class="rounded-lg p-2 text-[11px]"
      ...
      @if (pendingLinks().length > 0) {
        <div>
          <p class="text-[10px] font-semibold uppercase tracking-wider text-[var(--color-warning)] mb-1.5">
