...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, FormsModule, IconComponent, StickyHeaderComponent],
  template: `
    <app-sticky-header
    ...
    <div class="w-full px-2 md:px-0 mt-4">
      ...
      <div class="flex items-center gap-2 mb-4">
        <div class="flex items-center gap-1.5 bg-surface border border-border rounded-lg px-2.5 py-1.5">
      ...
      <div *ngIf="!loading && rules" class="payroll-grid">
        <div *ngFor="let card of cards" class="payroll-card bg-surface border border-border rounded-xl overflow-hidden">
          <!-- Card Header with Icon -->
          <div class="flex items-center gap-2.5 px-4 py-3 md:px-5 md:py-4 border-b border-border">
            <div
          ...
          <div class="p-3 md:p-4">
            <ng-container *ngFor="let sub of card.subsections; let last = last">
              <h4 class="text-[10px] font-semibold text-text-muted uppercase tracking-wider mb-2">{{ sub.title }}</h4>
              <div class="grid grid-cols-2 md:grid-cols-3 gap-2.5 md:gap-3">
                <div *ngFor="let field of sub.fields" class="relative">
                  <label class="block text-[11px] font-medium text-text-secondary mb-1 leading-tight">
                    {{ field.label }}
                    <span
                  ...
                  <div class="relative">
                    <ng-container *ngIf="field.type === 'currency'">
                      <span class="absolute left-2.5 top-1/2 -translate-y-1/2 text-[10px] text-text-muted font-medium">$</span>
                      <input
                        type="number"
                    ...
                    <ng-container *ngIf="field.type === 'percent'">
                      <input
                        ...
                        step="0.01"
                      />
                      <span class="absolute right-2.5 top-1/2 -translate-y-1/2 text-[10px] text-text-muted font-medium">%</span>
                    </ng-container>
              ...
              <hr *ngIf="!last || card.includeArl" class="border-t border-border/60 my-3 md:my-4" />
            </ng-container>

            <!-- ARL Rates (only in Seguridad Social card) -->
            <ng-container *ngIf="card.includeArl">
              <h4 class="text-[10px] font-semibold text-text-muted uppercase tracking-wider mb-2">Tarifas ARL por Nivel de Riesgo</h4>
...
export class PayrollSettingsComponent implements OnInit, OnDestroy {
  ...
  headerActions: StickyHeaderActionButton[] = [
    { id: 'save', label: 'Guardar', variant: 'primary', icon: 'save', loading: false, disabled: true },
  ];
      ...
      subsections: [
        {
          ...
          fields: [
            { key: 'minimum_wage', label: 'Salario Mínimo (COP)', type: 'currency', readonly: false },
            { key: 'transport_subsidy', label: 'Auxilio de Transporte (COP)', type: 'currency', readonly: false },
          ],
        },
        {
          title: 'Umbrales',
          fields: [
            { key: 'transport_subsidy_threshold', label: 'Umbral Transporte (×SMMLV)', type: 'number', readonly: false },
            { key: 'retention_exempt_threshold', label: 'Umbral Retención (×SMMLV)', type: 'number', readonly: false },
          ],
        },
        {
          title: 'Calendario',
          fields: [
            { key: 'days_per_month', label: 'Días por Mes', type: 'number', readonly: true },
            { key: 'days_per_year', label: 'Días por Año', type: 'number', readonly: true },
          ],
        },
      ],
    },
      ...
      subsections: [
        {
          title: 'Provisiones',
          fields: [
            { key: 'severance_rate', label: 'Cesantías', type: 'percent', readonly: false },
            { key: 'severance_interest_rate', label: 'Intereses Cesantías', type: 'percent', readonly: false },
            { key: 'vacation_rate', label: 'Vacaciones', type: 'percent', readonly: false },
            { key: 'bonus_rate', label: 'Prima', type: 'percent', readonly: false },
          ],
        },
      ],
    },
      ...
      subsections: [
        {
          ...
          fields: [
            { key: 'health_employee_rate', label: 'Salud', type: 'percent', readonly: false },
            { key: 'pension_employee_rate', label: 'Pensión', type: 'percent', readonly: false },
          ],
        },
        {
          title: 'Aportes Empleador',
          fields: [
            { key: 'health_employer_rate', label: 'Salud', type: 'percent', readonly: false },
            { key: 'pension_employer_rate', label: 'Pensión', type: 'percent', readonly: false },
            { key: 'sena_rate', label: 'SENA', type: 'percent', readonly: false },
            { key: 'icbf_rate', label: 'ICBF', type: 'percent', readonly: false },
            { key: 'compensation_fund_rate', label: 'Caja de Compensación', type: 'percent', readonly: false },
          ],
        },
      ],
    },
  ...
  private loadYears(): void {
    this.payrollService.getConfiguredYears()
  ...
  private loadRules(): void {
    this.loading = true;
    this.payrollService.getPayrollRules(+this.selectedYear)
      .pipe(
  ...
  onArlChange(level: number, percent: number): void {
    ...
    this.editedFields.arl_rates = { ...this.editedFields.arl_rates, [level]: decimal };
    this.hasChanges = true;
    this.updateHeaderActions();
  }
  ...
  isArlModified(level: number): boolean {
    return !!this.editedFields.arl_rates && level in this.editedFields.arl_rates;
  }
  ...
  saveChanges(): void {
    ...
    this.saving = true;
    this.updateHeaderActions();
    this.payrollService.updatePayrollRules(+this.selectedYear, this.editedFields)
      .pipe(
        takeUntil(this.destroy$),
        finalize(() => {
          this.saving = false;
      ...
      .subscribe({
        next: (res) => {
          ...
          this.hasChanges = false;
          this.updateHeaderActions();
          this.toastService.success('Reglas de nómina actualizadas exitosamente');
        },
        error: () => {
