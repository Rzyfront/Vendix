 ...
 *
 * Antes (Fase E): estaba envuelto en `<app-modal>` y se abría como diálogo.
 * Ahora (Fase F): la página estándar `ResolutionsPageComponent` vive en
 * `/admin/invoicing/resolutions`. Este componente se conserva como vista
 * interna (lista compacta) reusable en otros contextos que necesiten listar
 * resoluciones sin el layout de página estándar.
 */
@Component({
            ...
            >
              <div class="flex-1 min-w-0">
                ...
                <div class="text-xs text-text-secondary space-y-0.5">
                  ...
                  <div>
                    Vigencia:
                    {{ resolution.valid_from | date: 'dd/MM/yyyy' : 'UTC' }} -
                    {{ resolution.valid_to | date: 'dd/MM/yyyy' : 'UTC' }}
                  </div>
                </div>
              </div>
...
export class ResolutionsComponent {
  ...
  private loading$: Observable<boolean> = this.store.select(
    selectResolutionsLoading,
  );
