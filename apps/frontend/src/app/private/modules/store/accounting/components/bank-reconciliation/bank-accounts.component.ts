        ...
        <div class="sticky top-[99px] z-10 bg-background px-2 py-1.5 -mt-[5px]
          ...
          <div class="flex flex-col gap-2 md:flex-row md:justify-between md:items-center md:gap-4">
            ...
            <div class="flex items-center gap-2 w-full md:w-auto">
              ...
              ></app-inputsearch>
              <app-button variant="outline" size="sm" (clicked)="openImportModal()">
                <app-icon name="upload" [size]="16" slot="icon"></app-icon>
                <span class="hidden sm:inline">Importar</span>
              </app-button>
              <app-button variant="outline" size="sm" (clicked)="navigateToReconciliations()">
        ...
        <div class="relative p-2 md:p-4">
          ...
          @if (filteredAccounts().length === 0) {
            <div class="flex flex-col items-center justify-center py-16 text-gray-400">
              ...
              <p class="text-sm">{{ searchTerm() ? 'Intenta con otro termino de busqueda.' : 'Crea tu primera cuenta bancaria para comenzar.' }}</p>
            </div>
          } @else {
            <div class="divide-y divide-border">
              @for (account of filteredAccounts(); track account.id) {
                    ...
                    <div class="min-w-0 flex-1">
                      <div class="flex items-center gap-2">
                        ...
                        <span class="text-[10px] font-bold uppercase px-1.5 py-0.5 rounded"
                              [class]="account.status === 'active' ? 'bg-emerald-50 text-emerald-600' : 'bg-gray-100 text-gray-400'">
                          {{ getStatusLabel(account.status) }}
                        </span>
                      </div>
                      <div class="flex items-center gap-2 mt-1">
                        <span class="text-xs text-gray-500">{{ account.bank_name }}</span>
                        <span class="text-xs font-mono text-gray-400">{{ account.account_number }}</span>
                      </div>
                      <div class="mt-1">
                        <span class="text-sm font-semibold" [class]="account.current_balance >= 0 ? 'text-emerald-600' : 'text-red-500'">
                          {{ account.current_balance | currency:'COP':'symbol-narrow':'1.0-0' }}
                        </span>
                        @if (account.unreconciled_count) {
                          <span class="ml-2 text-[10px] bg-amber-100 text-amber-700 px-1.5 py-0.5 rounded-full font-bold">
                ...
                <div class="hidden md:grid md:grid-cols-12 gap-2 px-4 py-2.5 items-center hover:bg-gray-50 transition-colors">
                  <div class="col-span-3 text-sm text-text-primary font-medium truncate">{{ account.name }}</div>
                  <div class="col-span-2 text-sm text-gray-600 truncate">{{ account.bank_name }}</div>
                  <div class="col-span-2 text-sm font-mono text-gray-500">{{ account.account_number }}</div>
                  <div class="col-span-2 text-sm text-right font-semibold" [class]="account.current_balance >= 0 ? 'text-emerald-600' : 'text-red-500'">
                    {{ account.current_balance | currency:'COP':'symbol-narrow':'1.0-0' }}
                    @if (account.unreconciled_count) {
                      <span class="ml-1 text-[10px] bg-amber-100 text-amber-700 px-1.5 py-0.5 rounded-full font-bold">
                  ...
                  <div class="col-span-1 text-center">
                    <span class="text-xs px-2 py-0.5 rounded-full"
                          [class]="account.status === 'active' ? 'bg-emerald-50 text-emerald-600' : account.status === 'closed' ? 'bg-red-50 text-red-500' : 'bg-gray-100 text-gray-400'">
                      {{ getStatusLabel(account.status) }}
                    </span>
                  </div>
...
export class BankAccountsComponent implements OnInit {
  ...
  stats = computed(() => {
    ...
    return {
      ...
      pending_transactions: all.reduce((sum, a) => sum + (a.unreconciled_count || 0), 0),
    };
  });
