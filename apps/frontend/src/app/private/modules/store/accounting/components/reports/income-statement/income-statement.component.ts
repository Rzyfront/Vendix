...
import { FiscalPeriod, IncomeStatementReport } from '../../../interfaces/accounting.interface';
import {
            ...
            <div class="space-y-4">
              ...
              <div class="bg-surface rounded-xl shadow-[0_2px_8px_rgba(0,0,0,0.07)] border border-border overflow-hidden">
                <div class="px-4 py-3 bg-emerald-50 border-b border-border">
                  <h3 class="text-sm font-bold text-emerald-800 uppercase">Ingresos</h3>
                </div>
                @if (report.revenue.accounts.length) {
                  <div class="divide-y divide-border">
                    @for (row of report.revenue.accounts; track row.account_id) {
                ...
                <div class="px-4 py-3 bg-emerald-50 border-t border-border flex justify-between">
                  <span class="text-sm font-bold text-emerald-800">Total Ingresos</span>
              ...
              <div class="bg-surface rounded-xl shadow-[0_2px_8px_rgba(0,0,0,0.07)] border border-border overflow-hidden">
                <div class="px-4 py-3 bg-red-50 border-b border-border">
                  <h3 class="text-sm font-bold text-red-800 uppercase">Gastos</h3>
                </div>
                @if (report.expenses.accounts.length) {
                  <div class="divide-y divide-border">
                    @for (row of report.expenses.accounts; track row.account_id) {
                      <div class="px-4 py-2 flex justify-between items-center">
                        <div>
                          <span class="text-xs font-mono text-gray-500 mr-2">{{ row.account_code }}</span>
                          <span class="text-sm">{{ row.account_name }}</span>
                        </div>
                        <span class="text-sm font-mono font-medium text-red-600">
                          {{ row.balance | number:'1.2-2' }}
                        </span>
                      </div>
                    }
                ...
                <div class="px-4 py-3 bg-red-50 border-t border-border flex justify-between">
                  <span class="text-sm font-bold text-red-800">Total Gastos</span>
              ...
              <div class="p-4 rounded-xl border-2"
                   [class]="report.net_income >= 0 ? 'bg-emerald-50 border-emerald-300' : 'bg-red-50 border-red-300'">
                <div class="flex items-center justify-between">
                  <div class="flex items-center gap-3">
                    <app-icon
                      [name]="report.net_income >= 0 ? 'trending-up' : 'trending-down'"
                      [size]="24"
                      [class]="report.net_income >= 0 ? 'text-emerald-600' : 'text-red-600'"
                    ></app-icon>
                    <span class="text-lg font-bold"
                          [class]="report.net_income >= 0 ? 'text-emerald-800' : 'text-red-800'">
                      {{ report.net_income >= 0 ? 'Ganancia Neta' : 'Pérdida Neta' }}
                    </span>
                  </div>
...
export class IncomeStatementComponent implements OnInit {
  ...
  report$: Observable<IncomeStatementReport | null> = this.store.select(selectIncomeStatement);
  ...
  loadReport(): void {
    if (this.selected_period_id) {
      this.store.dispatch(loadIncomeStatement({
