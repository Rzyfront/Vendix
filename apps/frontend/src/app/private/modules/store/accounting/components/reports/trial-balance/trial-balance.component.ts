...
import { FiscalPeriod, TrialBalanceReport } from '../../../interfaces/accounting.interface';
import {
  selectTrialBalance,
  selectReportLoading,
  selectFiscalPeriods,
            ...
            @if (report.rows.length === 0) {
              ...
              <div class="divide-y divide-border">
                @for (row of report.rows; track row.account_id) {
                  <!-- Mobile -->
                  <div class="md:hidden p-3 mx-0.5 my-1 bg-surface rounded-2xl shadow-[0_2px_8px_rgba(0,0,0,0.07)]">
                    <div class="flex justify-between items-start">
                      <div>
                        <p class="text-xs font-mono text-gray-500">{{ row.account_code }}</p>
                        <p class="text-[15px] font-bold text-text-primary">{{ row.account_name }}</p>
                      </div>
                      <div class="text-right">
                        <p class="text-[10px] font-bold uppercase text-gray-500">D: {{ row.debit_total | number:'1.2-2' }}</p>
                        <p class="text-[10px] font-bold uppercase text-gray-500">C: {{ row.credit_total | number:'1.2-2' }}</p>
                        <p class="text-sm font-bold font-mono" [class]="row.balance >= 0 ? 'text-blue-600' : 'text-red-600'">
                  ...
                  <div class="hidden md:grid md:grid-cols-12 gap-2 px-4 py-2 items-center hover:bg-gray-50 transition-colors">
                    <div class="col-span-2 text-sm font-mono text-gray-600">{{ row.account_code }}</div>
                    <div class="col-span-4 text-sm">{{ row.account_name }}</div>
                    <div class="col-span-1 text-xs text-gray-500 capitalize">{{ row.account_type }}</div>
                    <div class="col-span-2 text-right text-sm font-mono">{{ row.debit_total | number:'1.2-2' }}</div>
                    <div class="col-span-2 text-right text-sm font-mono">{{ row.credit_total | number:'1.2-2' }}</div>
                    <div class="col-span-1 text-right text-sm font-mono font-bold"
                         [class]="row.balance >= 0 ? 'text-blue-600' : 'text-red-600'">
                      {{ row.balance | number:'1.2-2' }}
                    </div>
                  </div>
                }
              ...
              <!-- Totals -->
              <div class="grid grid-cols-12 gap-2 px-4 py-3 bg-gray-50 rounded-lg mt-2">
                <div class="col-span-7 text-sm font-bold text-text-primary">TOTALES</div>
                <div class="col-span-2 text-right text-sm font-mono font-bold">{{ report.total_debit | number:'1.2-2' }}</div>
                <div class="col-span-2 text-right text-sm font-mono font-bold">{{ report.total_credit | number:'1.2-2' }}</div>
...
export class TrialBalanceComponent implements OnInit {
  ...
  report$: Observable<TrialBalanceReport | null> = this.store.select(selectTrialBalance);
  ...
  loadReport(): void {
    if (this.selected_period_id) {
      this.store.dispatch(loadTrialBalance({
