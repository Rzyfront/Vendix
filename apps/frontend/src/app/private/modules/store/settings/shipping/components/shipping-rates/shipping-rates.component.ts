    ...
    >
      <div slot="header">
        <div class="w-10 h-10 rounded-xl bg-[var(--color-primary)]/10 flex items-center justify-center border border-[var(--color-primary)]/20">
          <app-icon name="dollar-sign" size="20" class="text-[var(--color-primary)]"></app-icon>
        </div>
      </div>
        ...
        <div class="flex-1 overflow-hidden flex flex-col md:flex-row">
          ...
          <div class="w-full md:w-[35%] lg:w-[40%] border-r border-[var(--color-border)] flex flex-col bg-white">
            <div class="p-4 border-b border-[var(--color-border)] flex justify-between items-center bg-gray-50/30">
               <h4 class="text-xs font-bold text-[var(--color-text-secondary)] uppercase tracking-widest">Tarifas Configuradas</h4>
            ...
            <div class="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">
              ...
              <div *ngIf="!loading && rates.length === 0" class="py-20 text-center px-8 border-2 border-dashed border-gray-100 rounded-2xl mx-2">
                 ...
                 <h5 class="text-sm font-bold text-gray-700">No hay tarifas aún</h5>
                 <p class="text-xs text-gray-400 mt-2">Crea reglas de envío personalizadas para esta zona geográfica.</p>
              </div>
              ...
              <div *ngFor="let rate of rates" 
                ...
                <div class="flex justify-between items-start mb-3">
                   <div>
                     ...
                     <h5 class="font-bold text-[var(--color-text-primary)]">{{ rate.name || 'Tarifa Estándar' }}</h5>
                   </div>
          ...
          <div class="w-full md:w-[35%] lg:w-[40%] flex flex-col bg-gray-50/50 border-r border-[var(--color-border)]">
            <div class="p-6 overflow-y-auto">
              <div class="flex items-center justify-between mb-8">
                <h4 class="text-lg font-black text-gray-900 flex items-center">
                   {{ selectedRate ? 'Editar Tarifa' : 'Nueva Configuración' }}
                   <span *ngIf="selectedRate" class="ml-3 text-[10px] font-bold bg-white px-2 py-1 rounded-lg border text-gray-400 tracking-widest">REF: {{ selectedRate.id }}</span>
                </h4>
              ...
              <form [formGroup]="form" id="shippingRateForm" (ngSubmit)="onSubmit()" class="space-y-4">
                  <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <app-selector
                      ...
                      [required]="true"
                      tooltipText="Selecciona a qué servicio de transporte se aplicará esta tarifa."
                    ></app-selector>
                    <app-input
                      label="Nombre Descriptivo"
                      formControlName="name"
                      placeholder="Ej: Envío Pesado"
                      tooltipText="Un nombre interno para identificar esta regla fácilmente."
            ...
            <div class="p-4 bg-white border-t border-[var(--color-border)] mt-auto">
               <app-button variant="primary" [fullWidth]="true" [loading]="isSubmitting" [disabled]="form.invalid" (clicked)="onSubmit()">
                  ...
                  {{ selectedRate ? 'Actualizar Tarifa' : 'Confirmar y Guardar' }}
               </app-button>
            </div>
          </div>
            ...
            <div class="p-5 space-y-6">
               <div class="space-y-4">
                 ...
                 <div class="p-3 bg-gray-50 rounded-xl space-y-2 border border-gray-100">
                    <p *ngFor="let ex of strategyHelp.examples" class="text-[10px] text-gray-500 leading-normal" [innerHTML]="ex"></p>
                 </div>
               </div>
               ...
               <div class="pt-6 space-y-3">
                  <h5 class="text-[10px] font-bold text-gray-400 uppercase tracking-widest">ProTip</h5>
                  <div class="p-3 bg-amber-50 rounded-xl border border-amber-100">
                    <p class="text-[10px] text-amber-700 leading-normal">
                      Crea rangos que no se solapen para evitar confusiones en el checkout del ecommerce.
                    </p>
                  </div>
               </div>
            </div>
...
export class ShippingRatesComponent implements OnInit, OnChanges {
  ...
  get minTooltip(): string {
    const type = this.form.get('type')?.value;
    if (type === ShippingRateType.WEIGHT_BASED) return 'Peso inicial (en kg) desde el cual se aplica esta tarifa.';
    if (type === ShippingRateType.PRICE_BASED) return 'Monto mínimo de compra necesario para aplicar esta tarifa.';
