...
import { CountryService, Country, Department } from '../../../../../../../services/country.service';
...
@Component({
  selector: 'app-shipping-zone-modal',
  standalone: true,
  ...
  template: `
    <app-modal
      [isOpen]="true"
      [title]="(zone ? 'Editar' : 'Nueva') + ' Zona de Envío'"
      [subtitle]="'Define el alcance geográfico de tus entregas'"
      (closed)="close.emit()"
      size="lg"
    >
      <div slot="header">
      ...
      <form [formGroup]="form" id="shippingZoneForm" (ngSubmit)="onSubmit()" class="space-y-4">
        <app-input
          label="Nombre de la Zona"
          formControlName="name"
          placeholder="Ej: Zona Norte, Todo Colombia..."
          [required]="true"
        ></app-input>
        ...
        <div>
          ...
          <p class="text-[10px] text-gray-400 mt-1.5 px-1 flex items-center gap-1">
            ...
            Actualmente se soporta un país por zona para una gestión regional optimizada.
          </p>
        </div>

        <!-- Regions / Departments -->
        <div *ngIf="showRegions" class="animate-in slide-in-from-top-2 duration-200 mt-6">
          <div class="flex items-center justify-between mb-4">
            <label class="block text-sm font-semibold text-[var(--color-text-secondary)] uppercase tracking-wide">Departamentos / Regiones</label>
            <div *ngIf="!loadingRegions" class="flex items-center gap-2">
               <span class="text-[10px] font-bold text-gray-400 bg-gray-100 px-2 py-0.5 rounded-md uppercase">
                 {{ selectedRegions.size }} seleccionados
               </span>
            </div>
          </div>
          
          <div *ngIf="loadingRegions" class="flex items-center justify-center p-12 bg-gray-50/50 rounded-2xl border border-dashed text-gray-400 gap-2">
            <app-icon name="loader-2" size="20" [spin]="true"></app-icon>
            <span class="text-sm font-medium">Cargando departamentos...</span>
          </div>

          <div *ngIf="!loadingRegions" class="border border-[var(--color-border)] rounded-2xl p-4 bg-gray-50/30">
             <div class="flex items-center justify-between mb-4 pb-3 border-b border-gray-200/60">
               <div class="flex items-center gap-2">
                 <input type="checkbox" [checked]="allRegionsSelected" (change)="toggleAllRegions($event)" id="all-regions" 
                 ...
                 <label for="all-regions" class="text-sm font-bold text-[var(--color-text-primary)] cursor-pointer">Seleccionar Todos</label>
               </div>
               <span class="text-xs text-gray-400 font-medium italic" *ngIf="selectedRegions.size === 0">Se asume "Todo el país"</span>
             </div>
             ...
             <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 max-h-60 overflow-y-auto pr-2 custom-scrollbar">
               <div *ngFor="let dep of departments" 
                 ...
                 <label [for]="'dep-' + dep.id" class="text-sm font-medium text-gray-700 cursor-pointer pointer-events-none">{{ dep.name }}</label>
               </div>
             </div>
          </div>
        </div>
        ...
        <div class="flex items-center justify-between p-4 rounded-xl border border-[var(--color-border)] bg-gray-50/30 mt-6">
           <div class="flex items-center gap-3">
             ...
             <div>
               ...
               <p class="text-xs text-[var(--color-text-secondary)]">Usa estas tarifas en el checkout.</p>
             </div>
           </div>
...
export class ShippingZoneModalComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.countryOptions = this.countries.map(c => ({ value: c.code, label: c.name }));

    if (this.zone) {
      this.form.patchValue({
        name: this.zone.name,
        country: this.zone.countries[0] || '', // Assuming single country for UI simplicity
  ...
  get allRegionsSelected(): boolean {
    return this.departments.length > 0 && this.selectedRegions.size === this.departments.length;
  }
  ...
  onSubmit() {
    ...
    const payload: Partial<ShippingZone> = {
      ...
      cities: [], // Future implementation
      zip_codes: [], // Future implementation
