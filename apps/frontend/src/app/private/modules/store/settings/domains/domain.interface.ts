export interface StoreDomain {
  ...
  domain_root_id?: number | null;
  root_hostname?: string | null;
...
export type StoreDomainType =
  | 'vendix_core'
  | 'organization'
  | 'store'
  | 'ecommerce';

export type StoreDomainAppType =
  | 'STORE_ECOMMERCE'
  | 'STORE_LANDING'
  | 'STORE_ADMIN';
...
export interface StoreDomainConfig {
  ...
  ssl?: {
    ...
    inherited_from_domain_root_id?: number;
...
export interface DomainRootAssignment {
  id: number;
  hostname: string;
  app_type: StoreDomainAppType | string;
  status: StoreDomainStatus | string;
  is_primary: boolean;
}
...
export interface DomainRoot {
  ...
  status: StoreDomainStatus | string;
  ssl_status: StoreDomainSSLStatus | string;
  routing_endpoint?: string | null;
  cloudfront_distribution_tenant_status?: string | null;
  last_verified_at?: string | null;
  assignments?: DomainRootAssignment[];
  created_at: string;
  updated_at: string;
}

export interface CreateDomainRootDto {
  hostname: string;
  config?: StoreDomainConfig;
}

export interface CreateDomainRootAssignmentDto {
  hostname: string;
  domain_type?: StoreDomainType;
  app_type?: StoreDomainAppType;
  is_primary?: boolean;
...
export interface DomainRootResponse {
  success: boolean;
  data: DomainRoot;
  message?: string;
}

export interface DomainRootsResponse {
  success: boolean;
  data: DomainRoot[];
  message?: string;
}
