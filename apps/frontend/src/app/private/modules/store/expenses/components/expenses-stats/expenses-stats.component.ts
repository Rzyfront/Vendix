...
export class ExpensesStatsComponent implements OnInit {
  ...
  summary$: Observable<ExpenseSummary | null> =
  ...
  pendingCount$ = this.summary$.pipe(
    map((s) => s?.counts_by_state?.pending || 0),
  );
  approvedCount$ = this.summary$.pipe(
    map((s) => s?.counts_by_state?.approved || 0),
  );
  paidCount$ = this.summary$.pipe(map((s) => s?.counts_by_state?.paid || 0));
  refundedCount$ = this.summary$.pipe(
    map((s) => s?.counts_by_state?.refunded || 0),
  );
