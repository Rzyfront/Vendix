...
import { GymPlansService } from '../../../plans/services';
...
@Component({
  selector: 'app-gym-membership-form-page',
  standalone: true,
  imports: [
    ReactiveFormsModule,
    StickyHeaderComponent,
...
export class GymMembershipFormPageComponent implements OnInit {
  ...
  onHeaderAction(actionId: string): void {
    if (actionId === 'cancel') this.router.navigate(['/admin/gym-ops/members']);
    ...
    this.membershipsService
      ...
      .subscribe({
        next: (membership) => {
          ...
          this.router.navigate(['/admin/gym-ops/members', membership.id]);
        },
        error: (err: unknown) => {
          this.isSubmitting.set(false);
