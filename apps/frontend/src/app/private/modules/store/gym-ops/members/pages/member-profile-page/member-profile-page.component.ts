...
import { GymMemberProfilesService, GymMembershipsService } from '../../services';
...
@Component({
  selector: 'app-gym-member-profile-page',
  standalone: true,
  imports: [
    ReactiveFormsModule,
    StickyHeaderComponent,
...
export class GymMemberProfilePageComponent implements OnInit {
  private readonly fb = inject(FormBuilder);
  private readonly profilesService = inject(GymMemberProfilesService);
  ...
  ngOnInit(): void {
    ...
    if (!Number.isFinite(customerId)) {
      this.router.navigate(['/admin/gym-ops/members']);
      return;
    }
  ...
  assignMembership(): void {
    this.router.navigate(['/admin/gym-ops/members/new'], {
      queryParams: { customer_id: this.customerId() },
    });
  }

  openMembership(m: GymMembership): void {
    this.router.navigate(['/admin/gym-ops/members', m.id]);
  }
