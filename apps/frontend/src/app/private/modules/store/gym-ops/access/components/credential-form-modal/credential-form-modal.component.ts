...
import { GymAccessService } from '../../services';
...
/**
 * Create / edit a gym access credential. On create, a customer + type + value
 * are required. On edit, only the value and active flag can change (the owner
 * and type are immutable). Emits `saved` when the write succeeds.
 */
@Component({
  selector: 'app-gym-credential-form-modal',
  standalone: true,
  imports: [
    ReactiveFormsModule,
    ModalComponent,
...
export class GymCredentialFormModalComponent {
  private readonly fb = inject(FormBuilder);
  private readonly accessService = inject(GymAccessService);
