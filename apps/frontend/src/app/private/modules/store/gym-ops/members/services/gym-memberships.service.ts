...
/**
 * Store-scoped service for gym memberships (Gym Suite — Ola 1).
 *
 * Mirrors `apps/backend/src/domains/store/gym-memberships`
 * (`/store/gym/memberships`). List/detail rows carry `plan` and `customer`
 * snapshots attached server-side.
 */
@Injectable({ providedIn: 'root' })
export class GymMembershipsService {
  private readonly apiUrl = environment.apiUrl;
  private readonly basePath = '/store/gym/memberships';
