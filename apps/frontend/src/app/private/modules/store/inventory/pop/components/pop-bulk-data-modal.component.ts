...
// Solo tipos. El runtime (7,2 MB) llega por `await import('xlsx')` en los dos
// puntos que de verdad lo usan: descargar la plantilla y leer el archivo.
...
export class PopBulkDataModalComponent {
  ...
  async downloadTemplate(type: 'quick' | 'complete') {
    const headers =
      type === 'complete'
        ? [
            'Nombre',
    ...
    let xlsx: typeof XLSX;
    try {
      xlsx = await import('xlsx');
    } catch {
      this.toastService.error('No se pudo cargar el generador de Excel. Reintenta.');
      return;
    }
