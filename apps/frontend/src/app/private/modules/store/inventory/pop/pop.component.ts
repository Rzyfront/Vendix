...
import { cartToPurchaseOrderRequest, CreatePurchaseOrderRequest } from './interfaces/pop-order.interface';
...
export class PopComponent implements OnInit, OnDestroy {
  ...
  onBulkDataReceived(items: any[]): void {
    ...
    items.forEach(row => {
      ...
      const name = normalizedRow['name'] || normalizedRow['nombre'] || normalizedRow['producto'] || normalizedRow['product'];

      // SKU: sku, code, codigo, id
      const sku = normalizedRow['sku'] || normalizedRow['code'] || normalizedRow['codigo'] || normalizedRow['id'];
      ...
      const costRaw = normalizedRow['cost'] || normalizedRow['costo'] || normalizedRow['price'] || normalizedRow['precio'] || 0;
      const unit_cost = Number(costRaw) || 0;

      // Quantity: quantity, qty, cantidad, cant
      const qtyRaw = normalizedRow['quantity'] || normalizedRow['qty'] || normalizedRow['cantidad'] || normalizedRow['cant'] || 1;
    ...
    if (addedCount > 0) {
      this.toastService.success(`Se importaron ${addedCount} productos al carrito`);
    } else {
      this.toastService.warning('No se encontraron productos válidos en el archivo (requiere Nombre y SKU)');
    }
  }

  onPrebulkAdded(event: { prebulkData: any, quantity: number, unit_cost: number, notes?: string }): void {
  ...
  onLotSave(lotInfo: any): void {
    if (this.currentLotItemId) {
      this.popCartService.updateItemLotInfo(this.currentLotItemId, lotInfo).subscribe();
    }
  ...
  onSubmitOrder(): void {
    ...
    if (!state.supplierId || !state.locationId || state.items.length === 0) {
      this.toastService.warning('Por favor complete los campos requeridos: proveedor, bodega y al menos un producto.');
      return;
    }
    ...
    this.purchaseOrdersService.createPurchaseOrder(request).subscribe({
      next: (response) => {
        if (response.success && response.data) {
          ...
          this.purchaseOrdersService.receivePurchaseOrder(orderId, receiveItems).subscribe({
            next: (res: any) => {
            ...
            error: (err: any) => {
              ...
              this.toastService.error('Orden creada pero hubo error al recibir stock');
