 ...
 * ('login' | 'register' | 'forgot') que opera como event bus (pub/sub) — caso
 * legitimo segun el skill `vendix-zoneless-signals` (no es estado UI). Se
 * conserva intencionalmente como Subject, no se migra a signal.
 */
@Injectable({
    providedIn: 'root',
})
export class StoreUiService {
    private readonly open_auth_modal_subject = new Subject<
    ...
    openForgotModal(): void {
        this.open_auth_modal_subject.next('forgot');
    }
}
