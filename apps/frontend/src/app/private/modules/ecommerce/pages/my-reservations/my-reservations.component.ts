...
import {
  ...
  CustomerBooking,
  AvailabilitySlot,
  RescheduleBookingDto} from '../../services/ecommerce-booking.service';
...
// State
  ...
  rescheduleSlotsForDate = computed(() => {
    ...
    if (!date) return [];
    return this.rescheduleSlots().filter(
...
loadBookings(): void {
  ...
  dateHasSlots(date: string): boolean {
    return this.rescheduleSlots().some(
      (s) => s.date === date && s.available > 0,
    );
  }
  ...
  checkIn(bookingId: number): void {
    this.http
      .post<any>(
        `${environment.apiUrl}/ecommerce/reservations/${bookingId}/check-in`,
        {},
      )
      .subscribe({
        next: () => {
