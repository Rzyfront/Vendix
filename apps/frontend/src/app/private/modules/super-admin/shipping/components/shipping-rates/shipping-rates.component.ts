    ...
    @if (zone()) {
      ...
      >
        ...
        <div class="flex-1 overflow-hidden flex flex-col md:flex-row">
              ...
              @if (!loading && rates.length === 0) {
                ...
                >
                  ...
                  <h5 class="text-sm font-bold text-gray-700">
                    No hay tarifas del sistema
                  </h5>
                  <p class="text-xs text-gray-400 mt-2">
                    Crea reglas de envío del sistema para esta zona geográfica.
                  </p>
                </div>
              }
                  ...
                  <div class="flex justify-between items-start mb-3">
                    <div>
                      ...
                      <span
                        ...
                        >{{ getMethodName(rate.shipping_method_id) }}</span
                      >
                      <h5 class="font-bold text-[var(--color-text-primary)]">
                        {{ rate.name || 'Tarifa del Sistema' }}
                      </h5>
                    </div>
                    <div class="text-right">
                      <span class="text-lg font-black text-emerald-600 block"
                        >\${{ rate.base_cost | number }}</span
                      >
                      <span
                        class="text-[10px] font-bold text-gray-400 uppercase tracking-widest"
                        >{{ rate.type.replace('_', ' ') }}</span
                      >
                    </div>
                  </div>
                  <div
          ...
          <!-- Column 2: Form (40%) -->
          <div
            class="w-full md:w-[35%] lg:w-[40%] flex flex-col bg-gray-50/50 border-r border-[var(--color-border)]"
          >
                  ...
                  <div class="grid grid-cols-2 lg:grid-cols-4 gap-2">
                    @for (
                      strategy of [
                        ...
                        {
                          type: ShippingRateType.WEIGHT_BASED,
                          label: 'Peso',
                          icon: 'package',
                        },
                        {
              ...
              <div class="pt-6 space-y-3">
                ...
                >
                  <p class="text-[10px] text-purple-700 leading-normal">
                    Esta tarifa del sistema estará disponible para todas las
                    tiendas que usen esta zona de envío.
                  </p>
                </div>
              </div>
            </div>
...
export class ShippingRatesComponent implements OnInit, OnChanges {
  ...
  get minTooltip(): string {
    const type = this.form.get('type')?.value;
    if (type === ShippingRateType.WEIGHT_BASED)
      return 'Peso inicial (en kg) desde el cual se aplica esta tarifa del sistema.';
    if (type === ShippingRateType.PRICE_BASED)
      return 'Monto mínimo de compra necesario para aplicar esta tarifa del sistema.';
  ...
  get strategyHelp(): {
    ...
    > = {
      [ShippingRateType.FLAT]: {
        ...
        description:
          'Se aplica un costo único sin importar el peso o el valor del carrito. Es la opción más sencilla para entregas locales a nivel sistema.',
        examples: [
