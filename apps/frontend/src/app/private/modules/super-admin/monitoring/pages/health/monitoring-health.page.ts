  ...
  template: `
    <div class="space-y-6">
      ...
      @if (serverInfo()?.memory) {
        <app-card [padding]="false" customClasses="!p-6">
...
export class MonitoringHealthPage {
  ...
  readonly appMetrics = signal<AppMetrics | null>(null);
  readonly serverInfo = signal<ServerInfo | null>(null);
  readonly loadingApp = signal(true);

  readonly hitRate = computed(() => {
    const redis = this.appMetrics()?.redis;
    if (!redis) return '0';
    const hits = redis.keyspaceHits;
    const misses = redis.keyspaceMisses;
