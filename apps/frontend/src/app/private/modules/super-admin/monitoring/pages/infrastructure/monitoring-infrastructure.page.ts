...
import { MetricChartComponent, TimeRangeSelectorComponent } from '../../components';
import { MonitoringService } from '../../services';
import { Ec2MetricsResponse, RdsMetricsResponse, TimeRange } from '../../interfaces';
...
export class MonitoringInfrastructurePage implements OnInit {
  ...
  private fetchEc2Metrics(): void {
    ...
    this.monitoringService.getEc2Metrics(this.ec2TimeRange).pipe(
      map(res => res.data),
      catchError(() => of(null)),
      takeUntilDestroyed(this.destroyRef),
    ).subscribe(data => {
  ...
  private fetchRdsMetrics(): void {
    ...
    this.cdr.markForCheck();
    this.monitoringService.getRdsMetrics(this.rdsTimeRange).pipe(
      map(res => res.data),
      catchError(() => of(null)),
      takeUntilDestroyed(this.destroyRef),
    ).subscribe(data => {
