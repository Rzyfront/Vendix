...
export const selectOrganizationName = createSelector(
  ...
  (organization: OrganizationConfig | null): string => organization?.name || '',
);

export const selectOrganizationSlug = createSelector(
  selectCurrentOrganization,
  (organization: OrganizationConfig | null): string => organization?.slug || '',
);
...
export const selectStoreName = createSelector(
  selectCurrentStore,
  (store: StoreConfig | null): string => store?.name || '',
);

export const selectStoreSlug = createSelector(
  selectCurrentStore,
  (store: StoreConfig | null): string => store?.slug || '',
);

// Domain info selectors
export const selectDomainHostname = createSelector(
  selectDomainConfig,
  (domainConfig: DomainConfig | null): string => domainConfig?.hostname || '',
);
