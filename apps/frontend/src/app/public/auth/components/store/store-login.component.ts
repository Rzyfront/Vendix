      ...
      <div class="max-w-md w-full space-y-8">
        ...
        <div class="text-center">
          <div class="mx-auto h-16 w-16 bg-primary rounded-full flex items-center justify-center mb-4" 
               *ngIf="storeLogo; else defaultLogo">
            <img [src]="storeLogo" [alt]="storeName" class="h-10 w-10">
          </div>
          ...
          <p class="mt-2 text-sm text-gray-600" *ngIf="storeName">
            en {{ storeName }}
          </p>
          <p class="mt-1 text-sm text-gray-500" *ngIf="!storeName">
            Tienda en línea
          </p>
        </div>
        ...
        <!-- Store Info -->
        <div class="text-center text-xs text-gray-500 mt-4" *ngIf="storeName">
          <p>Acceso a {{ storeName }}</p>
...
export class StoreLoginComponent implements OnInit, OnDestroy {
  loginForm: FormGroup;
  ...
  ngOnInit(): void {
    ...
    this.authFacade.error$.pipe(takeUntil(this.destroy$)).subscribe(error => {
      console.log('Store Login component - Error received:', error);
      if (error) {
    ...
    this.authFacade.isAuthenticated$.pipe(takeUntil(this.destroy$)).subscribe(isAuth => {
      if (isAuth) {
        console.log('Store user authenticated, redirecting...');
        this.toast.success(`¡Bienvenido a ${this.storeName || 'nuestra tienda'}!`);
      }
    });
  }
  ...
  private loadStoreContext(): void {
    this.tenantFacade.tenantConfig$.pipe(takeUntil(this.destroy$)).subscribe(tenantConfig => {
      if (tenantConfig?.store) {
        this.storeName = tenantConfig.store.name;
        this.storeLogo = tenantConfig.branding.logo.url || '';
      }
      ...
      // Apply store branding if available
  ...
  private handleLoginError(error: any): void {
    console.log('Store handleLoginError called with:', error);
    this.loginAttempts++;
    ...
    if (errorMessage.toLowerCase().includes('credenciales inválidas') || errorMessage.toLowerCase().includes('invalid credentials')) {
      ...
      this.setLoginError({
        ...
        message: 'Cuenta suspendida. Contacta al administrador de la tienda.',
        canRetry: false
      });
