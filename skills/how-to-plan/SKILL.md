...
| `agent-browser` (frontend E2E) | User-facing flow: login, navigation, form submit, render — against the real vhost | `agent-browser open https://vendix.com --headed --ignore-certificate-errors && agent-browser snapshot -i` (see `how-to-test`) |
...
> `app_type` by hostname. **Always pass `headed: true` + `extraArgs: ['--ignore-certificate-errors']`**
> in MCP calls (or `--headed --ignore-certificate-errors` flags in CLI) — Vendix uses a self-signed
> cert and the MCP defaults are headless + cert-strict. The full curl + agent-browser methodology (install, vhost setup,
