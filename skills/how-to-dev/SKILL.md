...
- **Mobile detection:** If the task involves `apps/mobile/`, load `mobil-dev` first — all edits are exclusive to `apps/mobile/`; other projects are read-only reference.
...
2. If the affected path is `apps/mobile/`, load `mobil-dev` — all edits are restricted to that directory.
