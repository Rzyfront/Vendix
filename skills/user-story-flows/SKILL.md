...
optionally published as a Claude Artifact for a shareable link.
...
5. **Verify.** Open the file in a browser (or publish as Artifact) and confirm: flows read L→R,
   branches render, animations run, reduced-motion disables them, mobile collapses to vertical,
   no external-resource requests in the console.
6. **(Optional) Publish** with the Artifact tool for a shareable link; keep the same file path so
