// Verificación visual del estado actual de Compras / POP (mobile parity)
// vs la versión web (pop.component.ts).
import { chromium } from 'playwright';

const SCREENSHOTS_DIR = '/tmp/verify-pop-parity';
...
async function run() {
  ...
  await page.goto(`${BASE_URL}/(store-admin)/inventory/pop`, { waitUntil: 'domcontentloaded', timeout: 30000 });
  await waitForAppReady(page);
  await page.waitForTimeout(5000); // POP carga products + suppliers + locations
  ...
  const checks = {
    // Header
    pageTitle: await page.locator('text=Punto de Compra').count(),
    popBadge: await page.locator('text=POP').count(),
    // Search bar
    searchBar: await page.locator('input[placeholder*="Buscar productos"]').count(),
    // Product grid (al menos 1 producto cargado)
    productCards: await page.locator('text=/\\$\\s?[\\d,.]+/').count(),
    // Footer actions (paridad web pop-mobile-footer / order confirmation)
    totalEstimado: await page.locator('text=Total Estimado').count(),
    verOrden: await page.locator('text=/^Ver orden$/').count(),
    borrador: await page.locator('text=/^Borrador$/').count(),
    crear: await page.locator('text=/^Crear$/').count(),
    crearRecibir: await page.locator('text=/Crear.*Recibir/').count(),
    // Body text contains expected terms
    hasBuscarProductos: bodyText.includes('Buscar productos') ? 1 : 0,
    hasProductosCargados: bodyText.includes('ACX') || bodyText.includes('ACC') ? 1 : 0,
  };

  console.log('\n=== Presencia de elementos ===');
  let pass = 0, fail = 0;
  for (const [k, v] of Object.entries(checks)) {
    const shouldBePresent = k !== 'errorBoundary';
    const present = v > 0;
