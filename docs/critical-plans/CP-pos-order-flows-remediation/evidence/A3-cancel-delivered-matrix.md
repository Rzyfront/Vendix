...
| Orden #1093 `refunded`, pago `partially_refunded`, ítem #1804 | Antes: **409 `ORD_ITEM_CANCEL_PAID_001`** (estado terminal tapado por pago); tras `2e5b2e107`: **409 `ORD_ITEM_CANCEL_STATE_001`**, `details.state=refunded`. | Orden sigue `refunded`, ítem sin `cancelled_at`; `order-flow.service.spec.ts` **122/122** y nuevo test de precedencia. El caso pagado #1091 conserva ERR-15. |
...
Jest `order-cancellation-policy.util.spec.ts` **46/46** y `order-flow.service.spec.ts` **122/122** tras el fix de precedencia. `SETTLED_PAYMENT_STATES` contiene succeeded/captured/partially_refunded/refunded, no pending. Código del guard corre antes de la transacción de recálculo.
