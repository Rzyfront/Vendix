...
- **Acceptance checklist:**
  - [x] Los cuatro carriles cobran el mismo producto sin impuesto con igual total, independientemente del área fiscal activa.
  ...
  - [x] Una línea antigua gravada conserva importe, `tax_rate_id` y `tax_type` aunque el catálogo actual no tenga asignación.
  - [x] Los carritos gravado, 0 % y mixto no pierden impuestos ni duplican orden/pago.
  - [x] Happy/sad/brute-force de API y los dos recorridos Playwright quedan documentados en `evidence/`.
  - [x] No se produjo ningún reporte que presuma «sin asignación = subdeclaración».
- **Status:** done · Fabio · 2026-09-23 · `evidence/H3-ui-mesas-taxless.md`, `evidence/H3-historic-snapshot.md`, `evidence/H3-store3-full-matrix.md`. API: cuatro carriles en tienda 10 INACTIVE y tienda 3 LOCKED, sin impuesto indebido ni pagos duplicados; gravado/0 %/mixto preservados. Playwright real en `vendix.com` (MCP no disponible) cobró el mismo producto 302 desde POS mesa (#1124) y Mesas (#1125), ambos $38000/IVA 0. Snapshot IVA19 antiguo siguió tipado tras retirar temporalmente asignación (#1126); se restauró la fila. La suscripción #4 y `enable_table_checkout` se restauraron a sus valores previos. Hallazgos secundarios de alias/aviso se corrigieron en pasos separados.
